...
* *Control plane*: This is where most cluster management, operations, and maintenance takes place. The control plane enforces rules in the data plane. You can use xref:security:authorization/rbac.adoc[role-based access control (RBAC)] in the control plane to manage access to resources (like resource groups, networks, and clusters) in your organization.

* *Data plane*: This is where your cluster lives. The term _data plane_ can be used interchangeably with _cluster_. The data plane is where you manage topics, consumer groups, connectors, and schemas. 
...
TIP: In the user interface, when you're at the organization (org) level or the resource group level, but you haven't yet selected a cluster, you're in the control plane. This is where you can select, create, and delete clusters, resource groups, and networks. When you're at the cluster level working with topics, consumer groups, connectors, and schemas, you're in the data plane.

=== BYOC architecture

In a BYOC architecture, you deploy the data plane in your own VPC. All network connections into the data plane take place through either a public endpoint, or for private clusters, through Redpanda Cloud network connections such as VPC peering, AWS PrivateLink, Azure Private Link, or GCP Private Service Connect. Sensitive data and credentials never leave the data plane. 
