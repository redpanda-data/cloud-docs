...
Clusters are configured and maintained in the control plane, but they remain available even if the network connection to the control plane is lost. You can use xref:security:authorization/rbac.adoc[role-based access control (RBAC)] in the control plane to manage and restrict access to resources (like resource groups, networks, and clusters) in your organization.
