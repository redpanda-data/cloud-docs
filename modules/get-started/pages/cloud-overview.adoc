...
Alternatively, you can contact https://redpanda.com/try-redpanda?section=enterprise-trial[Redpanda sales^] to request a private offer for monthly or annual committed use. With a usage-based billing commitment, you sign up for a monthly or an annual minimum spend amount. You can then use xref:billing:aws-commit.adoc[AWS Marketplace], xref:billing:azure-commit.adoc[Azure Marketplace], or xref:billing:gcp-commit.adoc[Google Cloud Marketplace] to quickly provision Dedicated clusters, and you can view invoices and manage your subscription in the marketplace.
