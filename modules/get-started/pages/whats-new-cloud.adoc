...
xref:get-started:cluster-types/serverless-pro.adoc[Serverless Pro] is a new cluster option. It is similar to Serverless Standard, but Serverless Pro supports higher usage limits and comes with Enterprise support. 

This is a limited availability (LA) release. It is a production-ready  option with automatically-scaling clusters available instantly. To start using Serverless Pro, https://redpanda.com/try-redpanda/cloud-trial#serverless[sign up for a free trial^]. This is no base cost, and with pay-as-you-go billing after the trial, you only pay for what you consume. 
