...
Redpanda Cloud increased the xref:get-started:create-topic.adoc[message size limits] on newly-created topics. BYOC and Dedicated clusters have a default message size limit of 20 MiB with a maximum of 32 MiB. Serverless clusters have a default message size limit of 8 MiB with a maximum of 20 MiB. Configure message size limits using the `max_message_bytes` topic property.
