...
xref:get-started:cluster-types/byoc/remote-read-replicas.adoc[Remote read replica] topics on AWS now can be deployed in a different region from the origin cluster's S3 bucket. This enables cross-region disaster recovery and data locality scenarios while maintaining the read-only replication model. 
