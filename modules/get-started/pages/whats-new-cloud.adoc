...
With xref:security:authorization/rbac.adoc[RBAC], you can assign users access to specific resources in your organization. For example, you can assign all users with a certain job read access on the entire organization but write access to only your non-production resource group. 
