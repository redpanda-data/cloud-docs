...
=== Unity Catalog support for BYOC clusters on AWS: GA

xref:manage:iceberg/iceberg-topics-databricks-unity.adoc[Unity Catalog integration] with Iceberg topics is now generally available (GA) for BYOC clusters on AWS. Unity Catalog is a unified governance solution for your data lakehouse, including managed Iceberg tables written from Iceberg-enabled topics in Redpanda.
