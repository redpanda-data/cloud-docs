...
=== Redpanda Connect: LA on AWS, Azure and GCP, and Serverless Beta

xref:develop:connect/about.adoc[Redpanda Connect] is now fully integrated into Redpanda Cloud and available as a fully managed service:

* For AWS, Azure, and GCP BYOC clusters as a limited availability release
* For Serverless clusters in Beta

xref:develop:connect/components/catalog.adoc[Choose from over 70 connectors] to quickly build and deploy streaming data pipelines or AI applications from the xref:develop:connect/connect-quickstart.adoc[Cloud UI] or using the xref:api:ROOT:cloud-api.adoc#tag--PipelineService[Control Plane API]. Comprehensive metrics and monitoring are also available.

To start using Redpanda Connect in the Cloud, xref:develop:connect/connect-quickstart.adoc[try this quickstart].
