...
ifdef::gbac-available[]
=== Group-based access control (GBAC)

xref:security:authorization/gbac.adoc[Group-based access control (GBAC)] lets you manage Redpanda permissions at the group level using OIDC. Assign OIDC groups to RBAC roles or create ACLs with `Group:<name>` principals. Because group membership is managed by your identity provider, onboarding and offboarding require no changes in Redpanda. GBAC is available for BYOC and Dedicated clusters.
endif::[]
