...
Use xref:security:authorization/rbac.adoc[RBAC] to assign users and service accounts roles for access to specific clusters or resource groups. This ensures users have the access necessary for their role, and it minimizes the risk of accidental or malicious data exposure.
