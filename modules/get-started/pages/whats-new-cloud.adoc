...
You can now xref:manage:cluster-maintenance/config-cluster.adoc[configure cluster configuration properties] with the Cloud API. For example, you can enable and manage Iceberg topics, data transforms, xref:manage:cluster-maintenance/audit-logging.adoc[audit logging], and xref:manage:schema-reg/schema-id-validation.adoc[server-side schema ID validation].
