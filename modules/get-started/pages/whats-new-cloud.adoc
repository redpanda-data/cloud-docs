...
New Serverless free-trial users now get a guided start in the Cloud UI. After you sign up, a few quick questions help Redpanda tailor your experience. The welcome cluster's *Overview* page summarizes your trial credits, and the *Get started* drawer offers guided ways to start streaming: create a Redpanda Connect pipeline, connect with your own Kafka client, or use `rpk` from the command line. See xref:get-started:cluster-types/serverless.adoc#get-started-with-serverless[Get started with Serverless].
