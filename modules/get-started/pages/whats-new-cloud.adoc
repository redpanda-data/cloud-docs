...
With xref:security:authorization/rbac/rbac.adoc[role-based access control (RBAC)] in the control plane, you can assign users access to specific resources in your Redpanda Cloud organization. For example, you could grant all users with a certain job title read access on the entire organization while limiting write access to only the non-production resource group. This alleviates the process of manually maintaining and verifying a set of ACLs for a large number of users. 

With RBAC in the data plane, you can manage access to topics and consumer groups. For more information, see xref:security:authorization/rbac/rbac_dp.adoc[].
