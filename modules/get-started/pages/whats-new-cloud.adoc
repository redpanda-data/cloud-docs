...
A Serverless cluster's *Overview* page now provides a quick way to get started with a xref:get-started:cluster-types/serverless.adoc#interact-with-your-cluster[Redpanda Connect pipeline]. This helps you stream data into and out of Redpanda without writing producer/consumer code. You can finish it in a couple minutes, or skip and come back later.
