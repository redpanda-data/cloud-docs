...
=== API gateway access for BYOC

BYOC clusters now include a setting for API gateway access:
...
* Private access restricts endpoint access to your VPC only.

If you change from public to private access after cluster creation, users without VPN access to your VPC will lose access to these services.
