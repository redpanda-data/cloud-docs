...
Enable two-factor authentication (2FA) to add an extra layer of security to your Redpanda Cloud account. After you enable 2FA, you'll enter your password (or use SSO), then be prompted for a one-time code from your authenticator app when you log in. Administrators can also xref:security:cloud-authentication.adoc#two-factor-authentication-2fa[enforce 2FA] for all members of an organization.
