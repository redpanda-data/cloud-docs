...
You can now edit xref:manage:cluster-maintenance/config-cluster.adoc[certain cluster configuration properties] with the Cloud API. For example, you can enable and manage Iceberg topics, data transforms, audit logging, and server-side schema ID validation.
