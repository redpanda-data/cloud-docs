...
For a better customer experience, the Serverless Standard and Serverless Pro products have merged into a single offering. xref:get-started:cluster-types/serverless-pro.adoc[Serverless clusters] now include the higher usage limits, additional AWS regions, and the free trial. 
