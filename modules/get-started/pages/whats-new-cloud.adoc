...
=== RBAC in the control plane and data plane

With xref:security:authorization/rbac/rbac.adoc[role-based access control (RBAC) in the control plane], you can manage access to organization-level resources like clusters, resource groups, and networks. For example, you could grant all users with a certain job title read access on the entire organization while limiting write access to only the non-production resource group. With xref:security:authorization/rbac/rbac_dp.adoc[RBAC in the data plane], you can configure cluster-level permissions for provisioned users at scale. 
