...
BYOC and Dedicated clusters with private networking now include a setting for API gateway access:

* Public access exposes Redpanda Console, Data Plane API, and MCP Server API endpoints over the internet, although they are still protected by authentication and authorization controls.
* Private access restricts endpoint access to your private network (VPC or VNet) only.

You can change the API gateway access setting on the cluster settings page. If you change from public to private access, users without VPN access to your VPC/VNet will lose access to these services.
