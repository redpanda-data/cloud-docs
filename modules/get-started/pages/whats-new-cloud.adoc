...
=== Redpanda SQL: GA on AWS BYOC

Redpanda SQL is generally available on BYOC clusters running on AWS. Run real-time SQL queries on Redpanda topic data, including the Iceberg history of Iceberg-enabled topics, using standard PostgreSQL syntax. Connect with `psql` or any PostgreSQL driver. See the xref:sql:get-started/sql-quickstart.adoc[quickstart guide] and xref:sql:get-started/overview.adoc[Redpanda SQL Overview].
