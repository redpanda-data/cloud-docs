...
You can now xref:manage:cluster-maintenance/config-cluster.adoc[configure cluster properties] with the Cloud API. For example, you can enable and manage xref:manage:iceberg/about-iceberg-topics.adoc[Iceberg topics], xref:develop:data-transforms/index.adoc[data transforms], and xref:manage:cluster-maintenance/audit-logging.adoc[audit logging]. 
