...
Redpanda Cloud increased the xref:get-started:create-topic.adoc[message size limits] on newly-created topics. BYOC and Dedicated clusters have a default message size limit of 20 MiB with a maximum of 32 MiB. Serverless clusters have a default message size limit of 8 MiB with a maximum of 20 MiB. This is configurable with the `max_message_bytes` topic property.

The message size setting on existing topics is not changed, but message size limits on existing topics can only be updated to the new maximums.
...
e-unified-redpanda-migrator.adoc[].
lities, making cloud operations more accessible through conversational interfaces.
