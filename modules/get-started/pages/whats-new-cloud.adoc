...
xref:get-started:cluster-types/byoc/azure/vnet-azure.adoc[BYOVNet for Azure] is now generally available (GA). This means that Redpanda is GA on Microsoft Azure for BYOC and BYOVNet clusters, including support for private networking with Azure Private Link.

=== BYOVPC on AWS: GA

xref:get-started:cluster-types/byoc/aws/vpc-byo-aws.adoc[BYOVPC for AWS] is now generally available (GA). This means that Bring Your Own networking (BYOVPC/BYOVNet) clusters are GA on all cloud providers: AWS, Azure, and GCP.
