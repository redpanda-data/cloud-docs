...
AI agents are configurable digital assistants designed to perform specialist tasks autonomously on your behalf. Redpanda's first AI agent is now available in beta for Dedicated and BYOC (not BYOVPC) clusters. Try out the xref:develop:agents/create-support-agent.adoc[support agent], which allows you to enrich the responses from a large language model (LLM) using data from your own GitHub repositories.
