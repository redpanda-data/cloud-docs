...
=== Get Started with Redpanda Connect on cluster overview

A cluster's *Overview* page now provides a *Get Started* wizard to quickly start streaming your own data with a Redpanda Connect pipeline. The wizard lets you stream data into and out of Redpanda without writing producer/consumer code. Advanced users can skip the wizard and go straight to the *Edit pipeline* step to configure the YAML file directly. See also: xref:develop:connect/connect-quickstart.adoc[Redpanda Connect quickstart].
