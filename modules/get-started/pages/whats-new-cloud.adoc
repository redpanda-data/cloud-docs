...
The Cloud API reference is now available as separate references for the xref:api:ROOT:cloud-controlplane-api.adoc[Control Plane API] and xref:api:ROOT:cloud-dataplane-api.adoc[Data Plane APIs]. The Control Plane API and Data Plane APIs follow separate OpenAPI specifications, and the reference is updated to better reflect the structure of the Cloud APIs and improve usability of the documentation. See also: xref:manage:api/cloud-api-overview.adoc[].
