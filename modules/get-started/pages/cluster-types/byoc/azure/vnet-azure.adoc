...
**Reviewers: we list the following limitations for AWS. Should we include them also for Azure?** 
...
* `vars.azure.tf`: This file contains the variables related to Azure credentials.
* `vars.condition.tf`: This file contains the conditional variables; for example, on whether to create NAT.
* `vars.customer_input.tf`: This file contains the variables needed to create a cluster with the Cloud API.
* `vars.iam.tf`: This file contains the variables related to IAM roles.
* `vars.misc.tf`: This file contains the variables, regions, zones, tags, and resource name prefix.
...
The output of `terraform apply` should display a number of output values. Note these values, because they are needed for the next steps. To continue with cluster creation, see https://github.com/redpanda-data/cloud-examples/tree/main/customer-managed/azure#create-azure-byo-vnet-redpanda-cluster[Create Azure VNet Redpanda Cluster^].

These values can also be retrieved by running `terraform output`. For example:
...
== Deploy customer-managed resources with Azure CLI

TODO

== Create a network with the Cloud API

Create a network with the POST body. Replace the variables with the actual values. To create a resource group and bearer token, follow the xref:manage:api/cloud-api-quickstart.adoc#try-the-cloud-api[Cloud API Quickstart].
...
A network ID is required to create a Redpanda cluster. Replace the variables with the actual values. To create a resource group and bearer token, follow the xref:manage:api/cloud-api-quickstart.adoc#try-the-cloud-api[Cloud API Quickstart].
...
After getting a Redpanda cluster ID, deploy the cluster with `rpk` by replacing `$rp_id` and `$subscription_id` with your values:
