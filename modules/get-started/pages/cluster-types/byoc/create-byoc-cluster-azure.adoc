...
. Update your cluster to enable `kafka_connect` by making a xref:api:ROOT:cloud-api.adoc#patch-/v1beta2/clusters/-cluster.id-[`PATCH /v1beta2/clusters`] request:

.. In `PATH PARAMETERS`, enter your cluster ID. You can find the cluster ID in the Redpanda Cloud UI on the **Clusters - Overview** page. 
.. In the body of the Update Cluster request, add:
+
```
  {
     "kafka_connect": {
