...
. Authenticate to the Redpanda Cloud API by going to xref:api:ROOT:cloud-api.adoc#auth[**Authentication**] in the sidebar, and clicking **Get Token**.
...
+
- `<cluster-id>`: Your cluster ID. You can find the cluster ID in the Redpanda Cloud UI under the Cluster Overview page in the Details section.
- `<token>`: The API token you received during authentication. This API token is in the header of the example request in the xref:api:ROOT:cloud-api.adoc#patch-/v1beta2/clusters/-cluster.id-[API reference].

. In your Azure subscription, disable the https://learn.microsoft.com/en-us/azure/key-vault/policy-reference[Key Vault secrets should have an expiration date^] policy.
