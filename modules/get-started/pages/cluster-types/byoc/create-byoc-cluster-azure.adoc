...
[%interactive]
- [ ] **Role**: The Azure user has the _Owner_ role in the subscription.
- [ ] **Quota**: The subscription has enough total regional vCPUs and vCPUs per instance family in the region where you will use Redpanda. See the https://learn.microsoft.com/en-us/azure/quotas/view-quotas[Microsoft documentation^].
+
...
- [ ] **Resources**: The subscription is registered for the following resource providers. See the https://learn.microsoft.com/en-us/azure/azure-resource-manager/management/resource-providers-and-types[Microsoft documentation^]. 
+
* Microsoft.Compute
...
```
- [ ] **Feature**: The subscription is registered for Microsoft.Compute/EncryptionAtHost. See the https://learn.microsoft.com/en-us/azure/virtual-machines/linux/disks-enable-host-based-encryption-cli#prerequisites[Microsoft documentation^]. 
+
To register it, run:
+
```
...
```
- [ ] **Monitoring**: The subscription has Azure Network Watcher enabled in the NetworkWatcherRG resource group and the region where you will use Redpanda. This lets you monitor and diagnose conditions at a network level. See the https://learn.microsoft.com/en-us/azure/network-watcher/network-watcher-create?tabs=portaly[Microsoft documentation^]. 
+
To enable it, run:
+
``` 
