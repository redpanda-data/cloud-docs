...
In the https://login.microsoftonline.com/[Azure Portal^], confirm that the dedicated subscription you will use with Redpanda has with the following: 
...
** Microsoft.Network
+
TIP: To check if an Azure resource provider is registered, run the following command using the Azure CLI or in the Azure Cloud Shell:
+
``` 
...
* Quotas: The subscription must have the following quotas in the region where you will use Redpanda. See the https://learn.microsoft.com/en-us/azure/quotas/view-quotas[Microsoft documentation^].
...
Check if the Azure subscription has SKU restrictions for the VM sizes in the region where you will use Redpanda. Using the Azure CLI or in the Azure Cloud Shell, run:
