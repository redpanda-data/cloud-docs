...
[NOTE]
====
Skip this step if you are enabling remote read replicas on standard BYOC clusters.
...
and `reader_cluster_id` variables in [cloud-examples](https://github.com/redpanda-data/cloud-examples/blob/main/customer-managed/aws/terraform/variables.tf)
...
[WARNING]
====
When using remote read replicas with customer-managed VPC Redpanda cannot guarantee deletion prevention. You are responsible for ensuring the source cluster is not deleted.
...
[NOTE]
====
Skip this step if you are enabling remote read replicas on customer-managed VPC clusters.
