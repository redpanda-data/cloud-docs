...
== Create your managed resources

Terraform code containing the specifications for the required resources can be found in https://github.com/redpanda-data/cloud-examples/tree/main/customer-managed/aws.
The ARNs output by this Terraform code are necessary in later steps.
...
> For more information about the fields in the Create Network request of the Cloud API see:
https://redpanda-api-documentation.netlify.app/#post-/v1beta2/networks
...
> For more information about the fields in the Create Cluster request of the Cloud API see:
https://redpanda-api-documentation.netlify.app/#post-/v1beta2/clusters
...
The `rpk cloud byoc aws apply` performs some validation checks before proceeding with provisioning.

RPK User: Checks if the user running the command has sufficient privileges for the provisioning of the agent. If any
permissions are found to be missing they will be displayed in the output.

IAM Instance Profile: Checks that the `connectors_node_group_instance_profile`, `redpanda_node_group_instance_profile`,
`utility_node_group_instance_profile`, and `k8s_cluster_role` have the minimum required permissions.  If any permissions
are found to be missing they will be displayed in the output.

Storage: Checks that the `management_bucket` exists and is versioned. Checks that the `cloud_storage_bucket` exists and
is not versioned. Checks that the `dynamodb_table` exists.

Network: Checks that the VPC exists. Checks that the subnets exist and have the expected tags. Checks that the security
groups exist and have the desired ingress and egress rules.

If there are validation errors that are believed to be erroneous you can rerun the command with the `--no-validate` tag
to skip validation.
