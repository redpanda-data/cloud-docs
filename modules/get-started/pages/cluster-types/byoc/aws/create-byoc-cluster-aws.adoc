...
[TIP]
====
To route all cluster egress through your own AWS Transit Gateway and hub VPC instead of a per-VPC NAT Gateway, set the *Transit Gateway ID* field on this page. This option is in beta and is only visible if centralized egress is enabled for your organization. See xref:networking:byoc/aws/nat-free-egress.adoc[].
====
. Click *Next*.
