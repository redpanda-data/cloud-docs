...
This topic explains how to create a Bring Your Own Virtual Private Cloud (BYOVPC) cluster on Amazon Web Services (AWS). This setup allows you to deploy the Redpanda glossterm:data plane[] into your existing VPC and manage the networking lifecycle. See also: xref:get-started:byoc-arch.adoc[BYOC architecture]. 
