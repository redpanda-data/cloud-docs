...
Define a JSON file called `byovnet.auto.tfvars.json` inside the Terraform directory that contains information about the VPC. Optionally, you can enable PrivateLink, and the `force_destroy_cloud_storage`, which destroys all existing cluster data when a cluster is created. For example: 
...
. For all cluster information, including the version, tier, storage, security, and availability zones, create the following environment variables with values from Terraform. 
+
```bash
