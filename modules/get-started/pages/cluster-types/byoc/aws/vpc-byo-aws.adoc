...
* You cannot move existing clusters to a BYOVPC cluster.
* You cannot change to a different VPC after creating a BYOVPC cluster.
...
Create a JSON file called `byovnet.auto.tfvars.json` inside the Terraform directory to configure variables for your specific needs. For details, see the https://github.com/redpanda-data/cloud-examples/blob/main/customer-managed/aws/README.md#configure-the-variables[Readme].
...
The output of `terraform apply` displays output values. Note these values. The ARNs (Amazon Resource Names) output by this Terraform code are necessary in later steps.

You can also get these values by running `terraform output`.
...
To create a cluster, issue a create cluster request using the Terraform outputs. Replace the variables with your actual values. 
...
To delete the cluster with the Cloud API, run:
