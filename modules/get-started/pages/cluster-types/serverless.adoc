...
Serverless Standard is the fastest and easiest way to start data streaming. A cluster is available instantly, there is no base cost, and you only pay for what you consume. For higher usage limits and Enterprise support, consider xref:get-started:cluster-types/serverless-pro.adoc[Serverless Pro]. For information about all Redpanda Cloud cluster types, see xref:get-started:cloud-overview.adoc[]. 
