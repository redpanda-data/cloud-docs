...
* Under the *Actions* dropdown, you can produce messages to topics. 
* Add team members and grant them access with glossterm:ACL[,access control lists (ACLs)] on the *Security* page.

On the cluster's *Overview* page, the *How to connect - CLI* tab lists steps to use `rpk` to interact with your cluster from the command line. Here are some helpful commands:
...
The *Overview* - *How to connect - Kafka API* tab lists your bootstrap server URL and security settings. This area includes code examples for creating a Kafka client to interact with your cluster.
