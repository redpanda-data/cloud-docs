...
. Select a cloud provider and region. See available regions in xref:reference:tiers/serverless-regions.adoc[Serverless regions]. For best performance, select the region closest to your applications. Redpanda expects your applications to be deployed in the same cloud provider and region as your Serverless cluster. 
+
Serverless clusters are not guaranteed to be pinned to a particular availability zone within the selected region.

== Interact with your cluster

A Serverless cluster's *Overview* page provides various ways to interact with your cluster:

Follow the *Get Started* wizard to quickly create a Redpanda Connect glossterm:pipeline[]. Pipelines help you stream data into and out of Redpanda without writing producer/consumer code. You can come back to here later to add more producers or consumers, or you can create a pipeline anytime on the *Connect* page. The get started wizard helps you select an input data source (to stream data into Redpanda) or output data sink (to stream data in Redpanda to an external system), and configure minimal settings. Open the Redpanda glossterm:topic[] to see messages streaming and verify the data flow.

The *How to connect - CLI* tab lists steps to use `rpk` to interact with your cluster from the command line. Here are some helpful commands:
