...
Redpanda Serverless is the fastest and easiest way to start data streaming. Serverless Pro is an enterprise-level version of xref:get-started:cluster-types/serverless.adoc[Serverless Standard] that comes with higher usage limits and Enterprise support. For information about all Redpanda Cloud cluster types, see xref:get-started:cloud-overview.adoc#redpanda-cloud-cluster-types[].
