...
IAM permissions allow Redpanda to access the cloud provider API to create and manage cluster resources, either through the Redpanda Cloud agent or, for new BYOC clusters on AWS, through a cross-account IAM role that Redpanda's control plane assumes. The permissions follow the principle of least privilege, limiting access to only what is necessary.
