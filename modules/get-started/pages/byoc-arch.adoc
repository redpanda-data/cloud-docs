...
Cross-account IAM delegation applies only to new clusters on AWS. Migrating an existing cluster is not supported, and the agent-based model described above continues to apply to existing clusters and to clusters on Azure and GCP. To deploy a new AWS cluster with cross-account IAM delegation, see xref:get-started:cluster-types/byoc/aws/cross-account-iam-aws.adoc[Deploy a BYOC Cluster on AWS with Cross-Account IAM Delegation]. For the permissions the role grants, see xref:security:authorization/cloud-iam-policies.adoc[AWS IAM policies].
