...
:page-aliases: ai-agents:mcp/local/quickstart.adoc
:page-beta: true
...
// Reader journey: "I'm new" - seeking first hands-on experience
...
In this quickstart, you'll get your Claude AI agent talking to Redpanda Cloud using the xref:develop:cloud-mcp/overview.adoc[Redpanda Cloud Management MCP Server].
...
+
TIP: For other clients, see xref:develop:cloud-mcp/configuration.adoc[].
...
NOTE: The MCP server does *not* expose API endpoints that result in delete operations by default. Use `--allow-delete` only if you intentionally want to enable delete operations. See xref:develop:cloud-mcp/configuration.adoc#enable_delete_operations[Enable delete operations].

:note-caption: Note

== Next steps

* xref:develop:cloud-mcp/configuration.adoc[]
