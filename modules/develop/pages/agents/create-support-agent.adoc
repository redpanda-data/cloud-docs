...
Learn how to create and configure a support agent. The support agent can use text-based content, such as such as Markdown or AsciiDoc, from any GitHub repository to enrich the responses given by an OpenAI model. You can then enter prompts from a chat interface and receive more accurate, domain-specific responses.

include::shared:partial$feature-flag-rpcn.adoc[]

== Overview

The support agent ingests data from a GitHub repository into a Redpanda topic, where the data is processed into vector embeddings. The vector embeddings are then written to a PostgreSQL vector database ready for retrieval by the agent. This is a type of Retrieval-Augmented Generation (RAG) pipeline, which continuously identifies and processes any changes to the GitHub repository so that the stored and retrieved data is always up-to-date.

The processing, storage, and retrieval of data is achieved using three preconfigured Redpanda Connect pipelines. The server resources used by these pipelines are charged at an xref:billing:billing.adoc#redpanda-connect-pipeline-metrics-dedicated-and-serverless[hourly rate in compute unit hours (compute/hour)].
...
- A PostgreSQL database with a vector extension enabled.
...
. Click on the **Support Agent** block.

== Configure your support agent

The first time you set up an agent, you must enter configuration details for most components. These details are then stored securely in the Redpanda Cloud's secret store so they are available when you add new agents or update existing ones.

Start by adding custom details to identify your agent and to guide how you want it to behave.

. In **Name**, enter a short name for the agent.
. In **Description**, enter a short description for the agent.
. In **System prompt**, add some short statements that describe what you want your agent to do.

Next, create a topic on your cluster for ingesting and processing the GitHub data, and add a new user with access to the topic. The user must have read or write access to the topic.
...
. Select a **SASL mechanism**. For more information, see  
...
. If your repository is private, also select **Private repository** and enter your personal access token.
. In **Include glob pattern**, specify the content you want the RAG pipeline to ingest, for example: all Markdown documents (`**/*.md`), all AsciiDoc documents (`**/*.adoc`), or everything beneath the `docs` directory (`/docs/**`).
...
. Click **Create**.
. In **OpenAI API key** token, select **+Create New** to add your OpenAI API secret key, which is used to process GitHub content into vector embeddings, and to send and retrieve responses from OpenAI.
. Enter a unique ID for the secret key, then paste in your secret key.
. Click **Create**.
. Click **Create** to create and run your agent. If the agent does not start, check the **Logs** under the **Git** and **RAG Indexing**, and **Agent** tabs to identify the issue.
...
To stop the pipelines, select each of the three tabs (**Git** and **RAG Indexing**, and **Agent**) and click **Stop**.
