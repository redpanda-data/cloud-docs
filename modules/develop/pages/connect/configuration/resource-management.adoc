...
You can allocate up to a maximum of 72 compute units per pipeline. You can add compute units in increments of one up to 15 units. Beyond this, scaling options increase to 33 units and then to 72 units. This scaling strategy is based on the number of machine cores required to provision these resources, which scale from two to four, and then to eight cores.
