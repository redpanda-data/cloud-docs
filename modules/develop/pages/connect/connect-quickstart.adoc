...
The *Connect* page lets you build data pipelines using a YAML editor. Add connectors from the left pane and configure them in the editor. Use slash commands (`/`) to insert variables, secrets, topics, and users.
...
. In the left pane, click to add the `generate` input connector and the `redpanda` output connector. The YAML for this connectorappears in the editor.
