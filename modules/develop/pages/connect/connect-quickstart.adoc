...
A Redpanda Cloud account for Serverless, Dedicated, standard BYOC, or BYOVPC on GCP. If you don't already have an account, https://redpanda.com/try-redpanda/cloud-trial[sign up for a free trial^].
...
+
--
Follow the instructions in xref:get-started:cluster-types/byoc/gcp/vpc-byo-gcp.adoc[Create a BYOVPC Cluster on GCP] to create and deploy a cluster called **connect-quickstart**.

NOTE: To enable Redpanda Connect on an existing BYOVPC cluster, see xref:get-started:cluster-types/byoc/gcp/enable-rpcn-byovpc-gcp.adoc[Enable Redpanda Connect on a BYOVPC Cluster on GCP]. 
