...
.. The **Configuration** section automatically populates the YAML with your selected components. To enhance the pipeline, replace the entire configuration with the following YAML, which adds names to the input and includes a `mapping` processor to convert the generated names to uppercase:
+
[source,yaml]
----
input:
