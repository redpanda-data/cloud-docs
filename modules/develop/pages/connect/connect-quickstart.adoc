...
+
Replace the `<cluster-password>` with the password of the connect user you set up in <<before-you-start,Before you start>>. The `${REDPANDA_BROKERS}` is a contextual variable that references the bootstrap server address of your cluster.
