...
== Enable egress for pipelines on BYOC clusters

The consumer pipeline in this quickstart drops messages instead of writing them to a real destination. When you replace the `drop` output with a connector that writes to your own systems, pipelines on BYOC clusters may need additional egress access.

On BYOC clusters, pipelines run inside the Redpanda dataplane VPC in your cloud account. By default, pipelines can connect to your Redpanda cluster and to publicly-routable endpoints. Outbound connections to other destinations, such as a database with a private address in a peered VPC, are blocked.
...
The allowlist permits outbound traffic from pipelines, but it does not create a network path to the destination. For private destinations, you must also establish routing, for example by peering the Redpanda dataplane VPC with the VPC that hosts your database.
...
* xref:connect:guides:getting_started.adoc[Configure, test, and run a data pipeline locally].
