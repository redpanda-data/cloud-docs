...
. **Add permissions**: Create a new user called `connect`. Leave the rest of the default settings. Click **Next**.
...
+
After a minute, select the pipeline and click **Stop** so you can examine the logs and see the processed messages. Similar to the first pipeline, you should see the active input and output types in the logs, and the rated jokes in the `dad-jokes` topic.
