...
TIP: To learn more about a component, hover over its name in the left pane to link to documentation.
...
. In the left pane, click *+* to *Add input*, and search for and select the `generate` input connector. The YAML for this connector appears in the editor. 

. Click *+* to *Add output*, and search for and select the `redpanda` output connector. The YAML for this connector also appears in the editor. 

. The "redpanda" connector requires a Redpanda topic and user, and a dialog prompts you to configure them. Select to create a new topic, and enter `dad-jokes` for the topic name. Then select to create a new user, and enter `connect` for the username. Click *Done* to create the topic and user and return to the pipeline editor.

. Replace the generated YAML in the editor with the following. This configuration includes the Redpanda topic and user as well as the `log` and `catch` processors and the `mapping` for joke generation:
+
[source,yaml]
----
input:
...
. Your pipeline details display, and after a few seconds, the pipeline starts running. If you don't see this change, refresh the page. The pipeline generates a new joke every five seconds and writes the joke to your topic.
...
. In the left pane, click *+* to *Add input*, and search for and select the `redpanda` input connector. 

. Click *+* to *Add output*, and search for and select the `drop` output connector. (For testing purposes, this output drops messages instead of forwarding them. In a real scenario you'd replace the `drop` connector with your real destination.)  
