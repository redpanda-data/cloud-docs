...
.. The **Configuration** section automatically populates the YAML with your selected components. You can add the `log` and `catch` processors to log generated jokes and monitor for errors.
+
For this quickstart, replace the entire configuration with the following YAML. This includes the processors and the input `mapping` for the joke generation:
+
[source,yaml]
----
input:
...
+
Once running, your pipeline will generate new jokes every five seconds and write the joke to your topic.
