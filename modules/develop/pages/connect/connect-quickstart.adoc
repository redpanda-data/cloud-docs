...
The *Connect* page lets you build data pipelines using a YAML editor. Add connectors and processors from the left pane and configure them in the editor.

[TIP]
====
* Click any connector to learn more about it in the documentation.
* Use slash commands (`/`) in the YAML editor to insert existing variables, secrets, topics, and users, or to create new ones.
...
+
TIP: In the YAML editor, you can type `/` to open the slash command menu. Use it to insert variables, secrets, topics, and users. You can select existing items or create new ones.
...
. On your pipeline page, switch to the **Live** mode to see the pipeline's activity log. The live mode loads new log messages as they come in. When Live mode is disabled, the log shows activity for from the past five hours. 
