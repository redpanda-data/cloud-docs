...
Improve account security by requiring a second verification step when signing in to Redpanda Cloud. Redpanda Cloud supports time-based one-time passwords (TOTP) using any compatible authenticator app (for example, Google Authenticator, Microsoft Authenticator, 1Password). 

With 2FA, after entering password or SSO verification at log in, you're prompted to scan the QR code to get a one-time code from your authenticator app. You can optionally select to remember the device to skip the 2FA prompt on that device for the next 30 days.
...
. Click *Enable* to set up two-factor authentication.
