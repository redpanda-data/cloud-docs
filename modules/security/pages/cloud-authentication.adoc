...
If your IdP provides OIDC group information, you can also use xref:security:authorization/gbac/index.adoc[group-based access control (GBAC)] to manage permissions at the group level. Register your IdP groups in Redpanda Cloud and assign roles to those groups, so that users automatically inherit permissions based on their group membership.
...
* No additional claims required for SSO: Redpanda Cloud SSO relies on the standard OIDC claims (`openid`, `profile`, `email`) provided by your IdP. You do not need to configure optional claims or group claims in ID/Access tokens.
* Group claims required for xref:security:authorization/gbac/index.adoc[group-based access control (GBAC)]: If you plan to use GBAC, you must configure your IdP to include group claims in OIDC tokens. 
...
. Create a service account in your organization, if you haven't already. In the Redpanda Cloud UI, go to the *Service account* tab of the https://cloud.redpanda.com/organization-iam?tab=service-accounts[Organization IAM^] page to create a service account. 
...
See xref:streaming:manage:security/authentication.adoc#mtls[Configure Authentication] for more details on principal mapping rules. The `principal_mapping_rules` field accepts a list of rules.
|===
====
...
When you consume, produce to, or manage topics using xref:streaming:reference:rpk/rpk-topic/rpk-topic.adoc[`rpk`], you must provide a client certificate and private key. You may use the `--tls-cert` and `--tls-key` options, or xref:streaming:reference:rpk/rpk-x-options.adoc[environment variables] with `rpk`.
