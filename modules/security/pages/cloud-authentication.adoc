...
To integrate with Entra ID, follow the https://learn.microsoft.com/en-us/entra/identity-platform/v2-protocols-oidc[Entra documentation^] to create an OIDC application and enable the ID token with Entra ID:

. In the https://www.microsoft.com/en-us/security/business/identity-access/microsoft-entra-id[Microsoft Entra App Gallery], in the Create your own application page: 
...
.. For Redirect URI, select *Web* platform with the Redpanda Cloud SSO Callback URL. To find this, in Redpanda Cloud, navigate to the Users page > Single sign-on tab, and click *Add connection*. Copy the *Callback URL*, and paste it into the Azure *Redirect URI* field. 
.. Click *Register*.
...
.. For *Realm*, enter your Azure email domain, such as: `cloudadminredpanda.onmicrosoft.com`.
.. Click *Save*.
.. On the Redpanda Cloud SSO page, edit your new Entra ID connection to enable SSO. 
+
Users with an email address with that realm can now access your Redpanda Cloud account. 
+
NOTE: You can continue to configure your Azure Enterprise applications page for Redpanda Cloud, but there is no need to configure JWT-based claims.
