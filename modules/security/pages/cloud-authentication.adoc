...
With MFA enabled, after entering your login credentials, you're prompted to scan a QR code to get a TOTP code from an authenticator app. Enter that 6-digit code to access Redpanda Cloud. You can choose to remember the device to skip the MFA prompt on that device for the next 30 days.

For the initial MFA setup, after entering that code, you're prompted to save a separate recovery code. Keep the recovery code offline and secure. You can use that recovery code to regain access to Redpanda Cloud, if necessary (for example, if your phone is lost). 
