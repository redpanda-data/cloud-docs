...
Improve account security by requiring a second verification step when signing in to Redpanda Cloud. Redpanda Cloud supports time-based one-time passwords (TOTP) using any compatible authenticator app (for example, Google Authenticator, Microsoft Authenticator, 1Password). If you enable 2FA, after password or SSO verification, users are prompted for a one-time code from their authenticator app when they sign in.

==== Enable 2FA (individual users)
...
. When the QR code appears, open your authenticator app, add a new account, and scan the QR code.
. Enter the verification code from your authenticator app to confirm setup.
. *(Optional)* Select *Remember this device for 30 days* to skip the 2FA prompt on this device for the next 30 days.

==== Enforce 2FA (organization admins)

Administrators can require 2FA for all users in an organization.

. In the Cloud UI, go to *Organization IAM*.
. Select the *2FA* tab.
. Click *Enable* to require 2FA for all members of this organization.

The next time users sign in to Redpanda Cloud, they'll be guided to scan a QR code with their authenticator app and enter the verification code before continuing. Users who haven't enrolled will be prompted to do so. They can optionally select to remember the device for 30 days to skip the 2FA prompt on that device for the next 30 days.
...
* *New phone or lost access:* If you can't access your authenticator app, use your recovery codes or contact your organization admin to help you regain access.
