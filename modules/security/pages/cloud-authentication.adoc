...
Improve account security by requiring a second verification step when signing in to Redpanda Cloud. Redpanda Cloud supports time-based one-time passwords (TOTP) using any compatible authenticator app (for example, Google Authenticator, Microsoft Authenticator, 1Password). After enabling 2FA, users are prompted for a one-time code from their authenticator app when they sign in.
...
* *New phone or lost access:* If you can’t access your authenticator app, contact your organization admin to help you regain access.
