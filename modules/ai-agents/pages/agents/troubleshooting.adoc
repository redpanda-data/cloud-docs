...
. Review conversation history in Inspector to identify unnecessary tool calls.
. Optimize tool implementations:
.. Add caching where appropriate
.. Reduce query complexity
.. Return only needed data (use pagination, filters)
. Clear conversation history in Inspector if context is very large.
...
. Clear conversation history periodically in Inspector.
