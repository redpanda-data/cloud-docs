...
On the Routers page, select the *LLM* tab to configure routing policies. The LLM routing pipeline represents the request lifecycle:
...
On the Routers page, select the *MCP* tab to configure tool discovery and execution. The MCP proxy aggregates multiple glossterm:MCP server[,MCP servers] behind a single endpoint, allowing agents to discover and call glossterm:MCP tool[,tools] through the gateway.
