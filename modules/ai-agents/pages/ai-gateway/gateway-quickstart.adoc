...
. Navigate to *Models*.
. Enable the models you want to use (for example, `gpt-4o`, `gpt-4o-mini`, `claude-sonnet-3.5`).
...
* Anthropic models: `anthropic/claude-sonnet-3.5`, `anthropic/claude-opus-4`
...
== Step 6: Configure LLM routing (optional)
...
. Add a second provider (for example, Anthropic) following Step 1.
...
== Step 7: Configure MCP tools (optional)
...
== Step 8: Create CEL routing rule (optional)
...
----
response = client.chat.completions.create(
    model="openai/gpt-4o",  # Will be routed based on CEL rule
...
After completing this quickstart, you:

* ✓ Configured an LLM provider and enabled models
* ✓ Created your first AI Gateway
* ✓ Sent requests through the gateway
* ✓ Verified requests in the observability dashboard
* ✓ (Optional) Configured failover with provider pools
* ✓ (Optional) Created CEL routing rules
* ✓ (Optional) Set up MCP tool aggregation
