...
:learning-objective-1: Write CEL expressions to route requests based on user tier or custom headers
:learning-objective-2: Test CEL routing logic using the UI editor or test requests
...
NOTE: To achieve true data residency, configure separate gateways per region with provider pools that meet your compliance requirements.
...
random() < 0.10
  ? "anthropic/claude-opus-4.6"  // 10% traffic to new model
  : "openai/gpt-5.2"             // 90% traffic to existing model
----
...
* 90% of requests -> Existing model (GPT-5.2)
...
# Check logs: ~10 should use opus-4.6, ~90 should use gpt-5.2
----
...
now().hour >= 22 || now().hour < 6  // 10pm - 6am
  ? "openai/gpt-5.2-mini"  // Off-peak: cheaper model
  : "openai/gpt-5.2"        // Peak hours: best model
----
...
1. Navigate to *Agentic* → *AI Gateway* → *Routers* → *Routing Rules*
...
rpk cloud ai-gateway test-cel \
  ...
  --expression 'request.headers["tier"] == "premium" ? "openai/gpt-5.2" : "openai/gpt-5.2-mini"' \
  --header 'tier: premium' \
