...
* Write CEL expressions to route requests based on user tier, environment, content complexity, or custom headers.
* Test CEL routing logic using the UI editor or test requests to verify expected model selection.
* Troubleshoot common CEL errors (type mismatches, missing fields, index out of bounds) using safe patterns.
