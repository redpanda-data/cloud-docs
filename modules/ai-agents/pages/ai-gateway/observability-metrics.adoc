...
* Monitor aggregate metrics (request volume, token usage, estimated spend) to track usage patterns and budget adherence.
* Compare model and provider performance using latency, error rate, and cost metrics.
* Configure alerts for budget thresholds and performance degradation.
...
2. Send test request (see xref:ai-agents:ai-gateway/quickstart-enhanced.adoc[])
