...
After xref:ai-agents:ai-gateway/ai-gateway.adoc[configuring your AI Gateway], set up Cursor IDE to route LLM requests and access MCP tools through the gateway's unified endpoints.
...
[source,text]
----
Override OpenAI Base URL: https://gw.ai.panda.com/v1
Override OpenAI API Key: YOUR_REDPANDA_API_KEY
----

[start=4]
. Scroll to *Advanced Settings* and add custom headers:
...
* `gpt-4o` - Routes to OpenAI GPT-4o through your gateway
* `gpt-4o-mini` - Routes to OpenAI GPT-4o-mini (cost-effective)
* `claude-sonnet-4-5` - Routes to Anthropic Claude Sonnet (if enabled in gateway)
* `claude-opus-4-5` - Routes to Anthropic Claude Opus (if enabled in gateway)
...
----
{
  "cursor.overrideOpenAIBaseUrl": "https://gw.ai.panda.com/v1",
  "cursor.overrideOpenAIApiKey": "YOUR_REDPANDA_API_KEY",
  "openai.additionalHeaders": {
    "rp-aigw-id": "GATEWAY_ID"
  },
  "cursor.cpp.defaultModel": "gpt-4o",
  "cursor.chat.defaultModel": "gpt-4o"
}
----
...
* `cursor.overrideOpenAIBaseUrl` - Gateway endpoint (always ends with `/v1` for OpenAI compatibility)
* `cursor.overrideOpenAIApiKey` - Your Redpanda API key (used for authentication)
* `openai.additionalHeaders` - Custom headers sent with every request
...
----
{
  "cursor.overrideOpenAIBaseUrl": "https://gw.staging.ai.panda.com/v1",
  "openai.additionalHeaders": {
...
----
{
  "cursor.overrideOpenAIBaseUrl": "${REDPANDA_GATEWAY_URL}/v1",
...
# Launch Cursor from terminal to inherit environment
cursor .
----
...
# Launch Cursor
& "C:\Users\YourName\AppData\Local\Programs\cursor\Cursor.exe"
----
  ...
  "cursor.mcp": {
    "servers": {
      "redpanda-ai-gateway": {
        ...
        "args": [
          "-e",
          "require('http').request({hostname:'gw.ai.panda.com',path:'/mcp',method:'GET',headers:{'Authorization':'Bearer YOUR_REDPANDA_API_KEY','rp-aigw-id':'GATEWAY_ID'}}).end()"
        ]
      }
    }
  }
...
This configuration uses Node.js to make HTTP requests to the gateway's MCP endpoint. The gateway returns tool definitions that Cursor can use.
...
** Model shows correct format (for example, `gpt-4o`)
...
* Autocomplete uses GPT-4o-mini for speed and cost efficiency
...
* `TEAM_GATEWAY_ID` - Gateway with team budget and rate limits
...
+
Verify the URL includes `/v1` at the end:
+
[,text]
----
# Correct
...
. **Missing headers**
+
Ensure `openai.additionalHeaders` is configured in settings:
+
[,json]
----
{
...
----
+
Smaller models like GPT-4o-mini or Claude Haiku provide faster responses ideal for autocomplete.
...
. **Missing header**
+
Ensure the `rp-aigw-id` header is in the `openai.additionalHeaders` section:
+
[,json]
----
{
...
----
{
  "cursor.cpp.defaultModel": "gpt-4o-mini"  // Fast model
}
----
...
* xref:ai-agents:ai-gateway/observability-logs.adoc[]: Monitor Cursor requests in the gateway dashboard
