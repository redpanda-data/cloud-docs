...
:description: Configure Redpanda AI Gateway to support Continue.dev clients with native provider formats, MCP tool aggregation, and authentication.
...
+
* `claude-opus-4-5` (Anthropic, high quality)
* `claude-sonnet-4-5` (Anthropic, balanced)
* `gpt-4o` (OpenAI, high quality)
* `gpt-4o-mini` (OpenAI, fast autocomplete)
...
* Autocomplete models (for example, `gpt-4o-mini`): 100 requests per minute per user
...
Continue.dev uses a JSON configuration file:
...
* Connects Continue.dev to the aggregated MCP endpoint
* Routes LLM requests through provider-specific backends
...
Continue.dev stores the API token in plain text in `config.json`. Remind users to:

* Never commit `config.json` to version control
* Use file system permissions to restrict access (for example, `chmod 600 ~/.continue/config.json`)
...
* **Headers not passed**: Confirm `requestOptions.headers` includes `rp-aigw-id`
...
* **Wrong model for autocomplete**: Use a fast model like `gpt-4o-mini` in `tabAutocompleteModel`
...
* **MCP configuration missing**: Ensure `experimental.modelContextProtocolServers` is configured
...
* **Autocomplete using expensive model**: Configure `tabAutocompleteModel` to use `gpt-4o-mini` instead of larger models
...
* **Chat using wrong model**: Route chat requests to cost-effective models (for example, `claude-sonnet-4-5` instead of `claude-opus-4-5`)
