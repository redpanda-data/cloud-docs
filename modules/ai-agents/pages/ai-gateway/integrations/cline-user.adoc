...
After xref:ai-agents:ai-gateway/ai-gateway.adoc[configuring your AI Gateway], set up Cline (formerly Claude Dev) to route LLM requests and access MCP tools through the gateway's unified endpoints.
...
. **Provider outage**
+
Check the AI Gateway dashboard for provider status. If the primary provider is down, configure failover (see xref:ai-agents:ai-gateway/quickstart-enhanced.adoc#step-6-configure-provider-pool-with-fallback[Configure failover]).
