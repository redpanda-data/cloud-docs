...
IMPORTANT: Continue.dev uses the `${{ secrets.* }}` syntax for interpolation in `config.yaml`. The legacy `${VAR}` syntax from `config.json` is not supported and will be treated as a literal string.
