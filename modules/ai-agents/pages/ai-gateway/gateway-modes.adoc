...
AI Hub mode provides instant, pre-configured access to OpenAI and Anthropic with zero setup complexity.

=== What it is

AI Hub mode eliminates complex LLM gateway configuration by providing pre-built routing rules and backend pools. Platform admins add provider credentials (OpenAI, Anthropic) once, and all teams immediately benefit from intelligent routing across both providers.
