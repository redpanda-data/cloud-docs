...
* Locate and filter request logs to debug specific failed requests or reconstruct user conversations.
* Interpret log fields (status codes, token usage, routing decisions) to diagnose performance and cost issues.
* Export logs for compliance auditing or long-term analysis.
...
| *Request Headers* 
| All headers sent 
| Includes `rp-aigw-id`, custom headers
