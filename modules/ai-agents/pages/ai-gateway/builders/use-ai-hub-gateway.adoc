...
AI Hub mode gateways provide instant access to OpenAI and Anthropic with pre-configured intelligent routing. As a builder, you benefit from zero-configuration access while your administrator manages provider credentials and routing policies.
...
* `openai/gpt-4o` → Always routes to OpenAI
* `anthropic/claude-sonnet-3.5` → Always routes to Anthropic

*Tier 2: Model Name Pattern*

When you specify a model without a prefix, AI Hub uses pattern matching:

* `gpt-4o` → Routes to OpenAI (matches `gpt-*` pattern)
* `claude-sonnet-3.5` → Routes to Anthropic (matches `claude-*` pattern)
...
|When enabled (default), `gpt-4o` routes to OpenAI and `claude-sonnet-3.5` routes to Anthropic without requiring vendor prefixes. When disabled, you must use explicit prefixes like `openai/gpt-4o`.
...
AI Hub gateways expose models from OpenAI and Anthropic based on your administrator's provider configuration.
...
* `anthropic/claude-opus-4` - Most capable Anthropic model
* `anthropic/claude-sonnet-3.5` - Balanced capability and cost
...
* *Gateway Endpoint*: Base URL (for example, `https://gw.ai.panda.com`)
* *Gateway ID*: Unique identifier (for example, `gw_abc123`)
...
# Configure client
client = OpenAI(
    base_url="https://gw.ai.panda.com",  # Gateway endpoint
    api_key="your-redpanda-api-key",      # Redpanda API key
    default_headers={
        "rp-aigw-id": "gw_abc123"         # Gateway ID
    }
)

# Send request
response = client.chat.completions.create(
    model="openai/gpt-4o-mini",           # Vendor/model format
    messages=[
...
model = "gpt-4o"  # Routes to OpenAI via pattern matching
model = "claude-sonnet-3.5"  # Routes to Anthropic via pattern matching
----
...
* Same authentication (Redpanda API key + gateway ID)
...
* *AI Hub*: OpenAI and Anthropic only
...
* Cannot integrate with providers other than OpenAI and Anthropic
