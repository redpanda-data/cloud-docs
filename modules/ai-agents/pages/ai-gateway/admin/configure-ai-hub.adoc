...
:learning-objective-3: Manage provider credentials for OpenAI and Anthropic

include::ai-agents:partial$ai-gateway-byoc-note.adoc[]

AI Hub mode provides instant, pre-configured access to OpenAI and Anthropic with zero setup complexity. Platform admins add provider credentials, and all teams immediately benefit from intelligent routing.
...
* [ ] Manage provider credentials for OpenAI and Anthropic
...
* 6 backend pools (OpenAI and Anthropic)
* 17 routing rules
...
Share the Gateway ID and Endpoint with teams who need to access this gateway.
...
* Model prefix missing: Use `anthropic/claude-sonnet-3.5` not `claude-sonnet-3.5`
