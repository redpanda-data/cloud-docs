...
* *Gateway ID*: Unchanged - applications continue using the same `rp-aigw-id`
...
[,cel]
----
// Route vision requests to GPT-4o (vision-capable)
request.headers["content-type"].contains("multipart") ? "openai/gpt-4o" : request.model
----
...
request.prompt.size() > 100000 ? "anthropic/claude-opus-4" : request.model
----
...
!request.model.contains("/") && !request.model.startsWith("gpt-") && !request.model.startsWith("claude-")
...
. Migrate applications to use new gateway (update `rp-aigw-id` header)
