...
:page-beta: true
:description: Learn how to configure the Redpanda Cloud MCP Server for your AI assistant, including auto and manual client setup, environment selection, enabling deletes, and security considerations.

This page explains how to configure the Redpanda Cloud MCP Server for your AI assistant, including auto and manual client setup, environment selection, enabling deletes, and security considerations.
...
For some supported clients, `rpk` can install and configure the MCP integration for you.For Claude and Claude Code, run one of these commands:
...
```
<1> The `--config` flag lets you target a specific `rpk.yaml`. Paths vary by OS and install method. Use an absolute path and ensure your user has read/write permissions.
...
* The `--config` flag lets you target a specific `rpk.yaml`. Paths vary by OS and install method. Use an absolute path and ensure your user has read/write permissions.
* For best results, keep your config files secure and never share them.

== Update or remove the MCP server

To update, re-run the `rpk cloud mcp install` command.
...
. [[local]]Start the server manually (one-time login required) to make sure it can reach Redpanda Cloud endpoints:
+
[source,bash]
----
rpk cloud login
