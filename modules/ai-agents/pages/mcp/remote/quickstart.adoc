...
:learning-objective-3: Connect Claude Code to your MCP server
:learning-objective-4: Use natural language to invoke your tools
...
* xref:ai-agents:mcp/remote/tool-patterns.adoc[]: Find reusable examples for database queries, Redpanda integration, and AI/LLM calls
* xref:ai-agents:mcp/remote/concepts.adoc[]: Learn how MCP tools differ from pipelines and when to use each component type
* xref:ai-agents:mcp/remote/admin-guide.adoc[]: Scale resources, monitor activity, and administer your MCP servers
* xref:ai-agents:mcp/remote/reference.adoc[]: Look up YAML rules, property restrictions, and MCP metadata options
