...
. <<pipeline-patterns, Implement the logic>>: Build the logic for your MCP tool using Redpanda Connect configurations.
...
* *Configuration*: This is where to put your YAML configuration to define an MCP tool.
...
* **Ownership**: `owner=platform-team`, `team=data-engineering`
* **Environment**: `env=production`, `env=staging`, `env=development`
* **Cost allocation**: `cost-center=marketing`, `project=analytics`
* **Purpose**: `purpose=customer-data`, `use-case=real-time-alerts`
...
* `secret_id`: Automatically assigned to track secrets used by the MCP server

If you attempt to use these reserved tag names when creating or updating an MCP server, the operation will fail with an error.
...
* Use tags for filtering and searching MCP servers in the console and API
...
[[pipeline-patterns]]
== Implement the logic of your MCP tool
