...
After defining the contract, implement the pipeline logic using Redpanda Connect processors, inputs, outputs, and caches.
...
Remote MCP tools are implemented as Redpanda Connect pipelines. This section shows you how to build different types of MCP tools using various pipeline patterns, organized by common use cases and integration scenarios.
...
* `output` (for data sinks)
* `processor` (for data transformations)
* `cache` (for caching intermediate results)

.Correct example for inputs
[source,yaml]
----
label: my_input
redpanda:
...
.Correct example for processors
[source,yaml]
----
processors:
