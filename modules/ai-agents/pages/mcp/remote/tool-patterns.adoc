...
:learning-objective-2: Look up integration patterns for external APIs and databases
:learning-objective-3: Find Redpanda topic integration patterns
:learning-objective-4: Identify production workflow patterns for complex scenarios

This page provides a catalog of patterns designed for use with MCP servers in Redpanda Cloud. Use these patterns as building blocks for your own MCP tools.
...
Each pattern is a reusable example for a common MCP tool scenario. Patterns are grouped by use case. All YAML is ready to use in your MCP server.

TIP: For step-by-step instructions, see xref:ai-agents:mcp/remote/create-tool.adoc[]. For best practices, see xref:ai-agents:mcp/remote/best-practices.adoc[]. For conceptual background, see xref:ai-agents:mcp/remote/concepts.adoc[].
...
* <<consume-from-redpanda-topics,Read from Redpanda topics>>
* <<data-transformation,Transform or validate data>>
* <<publish-to-redpanda-topics,Publish to Redpanda topics>>
* <<stream-processing,Process streaming data>>
...
include::redpanda-connect:ai-agents:partial$mcp/production-workflows.adoc[]
