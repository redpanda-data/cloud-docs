...
:page-aliases: mcp/local/overview.adoc, mcp/local/index.adoc
:page-beta: true
...
. Configure your MCP client using the `rpk cloud mcp install` command. Your client then starts the server on-demand using `rpk cloud mcp stdio`, authenticating with the Redpanda Cloud token from `rpk cloud login`.
