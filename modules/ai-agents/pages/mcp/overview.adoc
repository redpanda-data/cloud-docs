= MCP Overview
:description: Learn about Model Context Protocol (MCP) in Redpanda Cloud, including the two complementary options: the Redpanda Cloud API MCP and Remote MCP.
...
Redpanda Cloud API MCP badge::[label=beta, tooltip={page-beta-text}]::
...
[cols="1h,2,2"]
|===
| | Redpanda Cloud API MCP | Remote MCP
...
**Use the Redpanda Cloud API MCP** when you want to:
...
You can use both options together. For example, use the Redpanda Cloud API MCP to create a cluster, then deploy Remote MCP tools to analyze data in that cluster.
