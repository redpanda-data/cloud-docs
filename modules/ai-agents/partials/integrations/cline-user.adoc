...
** Gateway endpoint URL, which includes the gateway ID (for example, `\https://ai.prd.cloud.redpanda.com/gateway/v1/chat/completions`)
...
. Set *Base URL*: Your gateway endpoint URL (for example, `\https://ai.prd.cloud.redpanda.com/gateway/v1/chat/completions`). The gateway ID is embedded in the URL path.
...
* `<your-gateway-endpoint>` - Your gateway endpoint URL (the gateway ID is embedded in the URL path)
...
----
{
  ...
  "cline.baseURL": "<your-gateway-endpoint>",
  "cline.model": "anthropic/claude-opus-4.6-5"
}
----
...
[,bash]
----
export GATEWAY_DEV_URL="<your-dev-gateway-endpoint>"
export GATEWAY_PROD_URL="<your-prod-gateway-endpoint>"
----
...
[,powershell]
----
$env:GATEWAY_DEV_URL = "<your-dev-gateway-endpoint>"
$env:GATEWAY_PROD_URL = "<your-prod-gateway-endpoint>"
----
...
# Incorrect
<your-gateway-endpoint>/v1
<your-gateway-endpoint>/chat/completions
----
+
...
+
Check that the gateway endpoint URL in your Cline configuration matches your gateway exactly. Copy it directly from the AI Gateway UI.
...
+
Verify that the gateway endpoint URL in your Cline configuration matches the gateway you're viewing in the dashboard.
...
|`anthropic/claude-sonnet-4.5` or `anthropic/claude-opus-4.6-5`
