...
NOTE: Claude Code supports `${VAR}` interpolation syntax in the `mcpServers` section. The variables `REDPANDA_GATEWAY_URL` and `REDPANDA_API_KEY` will be resolved from environment variables at runtime.
