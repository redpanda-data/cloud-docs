...
** Gateway endpoint URL (for example, `\https://gw-abc123.ai.panda.com`)
...
* `<your-gateway-endpoint>/mcp` - Your gateway's MCP endpoint
...
* `<your-gateway-endpoint>` - Your gateway endpoint URL
* `YOUR_API_KEY` - Your Redpanda API key
...
----
{
  "mcpServers": {
    "redpanda-staging": {
      "type": "http",
      "url": "<your-staging-gateway-endpoint>/mcp",
      "headers": {
    ...
    "redpanda-production": {
      "type": "http",
      "url": "<your-production-gateway-endpoint>/mcp",
      "headers": {
...
For sensitive credentials, use environment variables instead of hardcoding values. Claude Code supports `${VAR}` interpolation in project-level `.mcp.json` files but not in `~/.claude.json`. To use environment variables, create a `.mcp.json` file in your project root:
...
[,powershell]
----
$env:REDPANDA_GATEWAY_URL = "<your-gateway-endpoint>"
...
+
Verify your gateway endpoint URL matches exactly. Copy it directly from the AI Gateway UI rather than typing it manually.
...
[,bash]
----
curl -I <your-gateway-endpoint>/mcp
----
+
...
+
Verify that the gateway endpoint URL in your configuration matches the gateway you're viewing in the dashboard.
