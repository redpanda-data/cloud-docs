...
Because Azure does not let you initiate a peering against another tenant without having delegated access to that tenant, Redpanda must first creates a multi-tenant Entra ID Enterprise application with a service principal attached to that application in Redpanda's tenant. The Redpanda Cloud UI provides this application ID, and then you must create a service principal on your tenant with that application ID, and attach a role to that service principal. Redpanda can then initiate the peering.
