...
The hub VPC attaches via its private subnets. This is where the Transit Gateway delivers inbound traffic into the hub.
...
. If the accounts are not in the same AWS Organization, in the Redpanda cluster account, open AWS RAM and accept the pending invitation under *Resource share invitations*. Redpanda accepts this invitation as part of cluster deployment.
...
| Transit Gateway attachment is stuck in `pending` | The Redpanda cluster account has not accepted the AWS RAM share invitation. Contact your account team if the invitation was not auto-accepted.
