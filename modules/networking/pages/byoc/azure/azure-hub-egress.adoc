...
You provision the hub VNet and Azure Firewall first, then give Redpanda the hub VNet ID and firewall private IP to create the BYOC network with centralized egress. The Redpanda spoke VNet's resource ID becomes available only after that network is created, so you create the hub-side peering (see <<peer-hub-vnet>>) after network creation, not before.
...
The <<automated-alternative,Terraform reference module>> does not set an explicit `zones` argument on the firewall or its public IP, so it inherits the same zone-redundant default in regions that support it. Only add explicit zones if you need a single-zone deployment or the Premium tier.
