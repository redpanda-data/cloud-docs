...
The following diagram shows the data flows used by private Redpanda clusters. 

image::shared:data-flows.png[Redpanda Cloud private cluster data flows]

==== Private network metadata flows

Metadata flows are the network traffic that carries metadata, such as topic and partition information and cluster configuration. Metadata flows are used by the Redpanda Cloud agent to manage the cluster and by clients to discover cluster resources.

The following diagram shows the metadata flows used by private Redpanda clusters. 

image::shared:metadata-flows.png[Redpanda Cloud private cluster metadata flows]

==== Private network control flows

Control flows are the network traffic that carries control messages, such as cluster management commands and configuration updates. Control flows are used by the Redpanda Cloud agent to manage the cluster and by clients to send administrative commands.

The following diagram shows the control flows used by private Redpanda clusters.
