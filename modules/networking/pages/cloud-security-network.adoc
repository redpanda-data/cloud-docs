...
The following network port is used for outgoing network connections outside the VPC. Redpanda does not include DNS or NTP ports, because those network flows do not leave the cloud provider's network, and they reach the internal cloud provider services within the VPC.
