...
* For BYOC and Dedicated clusters on AWS and GCP, Redpanda provisions one NAT gateway and one internet gateway. 
* For BYOC and Dedicated clusters on Azure, Redpanda provisions one NAT gateway and one public IP prefix of 31 bits.
* For BYOVPC and BYOVNet, you decide how to provide access to the internet, because you fully manage the network.
...
NOTE: For BYOC clusters (including BYOVPC/BYOVNet), you can find the NAT gateway IP address of your cluster in the Redpanda Cloud UI. You can add this IP address to the allowlist of your data source. While Redpanda Data does not guarantee that the NAT gateway IP will remain static, it is unlikely to change.
