...
Public Redpanda clusters deploy networks segmented by workload type. The Redpanda Cloud agent gets its own subnet as well as Redpanda's brokers and Redpanda Connect nodes. Public clusters deploy brokers in public subnets. Redpanda ports are protected by SASL/SCRAM authentication (SCRAM-SHA-256, SCRAM-SHA-512) and encrypted in transit using TLS 1.2. Everything else is deployed on private subnets.
