...
Redpanda Connect and Kafka Connect connectors that egress to the internet can incur NAT data transfer costs. You can add the NAT gateway IP address to your data source allowlist, if needed. While Redpanda Data does not guarantee the address will remain static, it is unlikely to change.

You can find the NAT gateway IP address on the cluster *Overview* page on the *Internet gateway* field. This field is available for BYOC clusters (including BYOVPC/BYOVNet) and Dedicated clusters.
