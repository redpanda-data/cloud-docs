...
NOTE: This guide is for configuring AWS PrivateLink on Serverless clusters using the Redpanda Cloud API. See xref:networking:serverless/aws/privatelink-ui.adoc[Configure PrivateLink in the Redpanda Cloud Console] if you want to set up the endpoint service using the Redpanda Cloud Console.
...
For example, if the bootstrap server URL is: `cki01qgth38kk81ard3g.any.us-east-1.aw.priv.prd.cloud.redpanda.com:9092`, then `cluster_domain` is: `cki01qgth38kk81ard3g.eu-west-1.aw.priv.cloud.redpanda.com](cki01qgth38kk81ard3g.any.us-east-1.aw.priv.prd.cloud.redpanda.com`.
...
include::networking:partial$private-links-access-rp-services-serverless.adoc[]
