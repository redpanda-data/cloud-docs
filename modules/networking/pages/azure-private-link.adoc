...
You can test the Private Link connection from any VM or container in the subscription where the endpoint is created. If configuring a Kafka client isn't possible right away, you can do these checks using xref:streaming:get-started:rpk-install.adoc[`rpk`] or cURL:
