...
#
#   Adjust the end port number if the Redpanda broker count is not 3.
#
aws ec2 authorize-security-group-ingress --region $REGION --profile $PROFILE \
    --group-id $SECURITY_GROUP_ID \
    --protocol "tcp" \
...
#   There are 3 ranges that alternate during normal system maintenance and all 3 must be open
#
#   Adjust the end port number if the Redpanda broker count is not 3.
#
aws ec2 authorize-security-group-ingress --region $REGION --profile $PROFILE \
