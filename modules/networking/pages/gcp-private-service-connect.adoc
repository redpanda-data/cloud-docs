...
rpk cloud byoc gcp apply --redpanda-id="${CLUSTER_ID}" --project-id='<service-project-id>'
``` 
- Make a request to the link:/api/doc/cloud-controlplane/operation/operation-clusterservice_updatecluster[`PATCH /v1/clusters/{cluster.id}`] endpoint to update the cluster to include the newly-created Private Service Connect NAT subnet.
+
[,bash]
----
