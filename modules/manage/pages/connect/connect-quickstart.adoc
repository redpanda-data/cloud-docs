...
This quickstart introduces you to Redpanda Connect fore Redpanda Cloud. Using a single YAML configuration file, Redpanda Connect lets you quickly build, run, and seamlessly update streaming data pipelines. No third-party connectors are required. 
...
| A xref:manage:connect/guides/bloblang/functions.adoc#fake-data-generation[fake data source] that generates a batch of email messages every second, populated with an ID, paragraph, email address, and a user name.
