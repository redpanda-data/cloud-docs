...
See link:/api/doc/cloud-dataplane/[Data Plane API] for the full Data Plane API reference documentation.

The link:/api/doc/cloud-dataplane/topic/topic-cloud-api-overview#topic-cloud-api-architecture[data plane] contains the actual Redpanda clusters. Every cluster is its own data plane, and so it has its own distinct link:/api/doc/cloud-dataplane/topic/topic-cloud-api-overview#topic-data-plane-apis-url[Data Plane API URL].
...
+
--
To retrieve the Data Plane API URL of a cluster, make a request to the link:/api/doc/cloud-controlplane/operation/operation-clusterservice_getcluster[`GET /v1/clusters/\{id}`] endpoint of the Control Plane API.
--

Serverless::
+
--
To retrieve the Data Plane API URL of a cluster, make a request to the link:/api/doc/cloud-controlplane/operation/operation-serverlessclusterservice_getserverlesscluster[`GET /v1/serverless/clusters/\{id}`] endpoint of the Control Plane API.
--
======
...
To create a new user in your Redpanda cluster, make a POST request to the link:/api/doc/cloud-dataplane/operation/operation-userservice_createuser[`/v1/users`] endpoint, including the SASL mechanism, username, and password in the request body:
...
To create a new ACL in your Redpanda cluster, make a link:/api/doc/cloud-dataplane/operation/operation-aclservice_createacl[`POST /v1/acls`] request. The following example ACL allows all operations on any Redpanda topic for a user with the name `payment-service`.
...
To create a new Redpanda topic without specifying any further parameters, such as the desired topic-level configuration or partition count, make a POST request to link:/api/doc/cloud-dataplane/operation/operation-topicservice_createtopic[`/v1/topics`] endpoint:
...
Make a request to link:/api/doc/cloud-dataplane/operation/operation-secretservice_updatesecret[`PUT /v1/secrets/\{id}`]. You can only update the secret value, not its name. You must use a Base64-encoded secret.
...
To get details of a specific pipeline, make a link:/api/doc/cloud-dataplane/operation/operation-redpandaconnectservice_getpipeline[`GET /v1/redpanda-connect/pipelines/\{id}]` request.
...
To stop a running pipeline, make a link:/api/doc/cloud-dataplane/operation/operation-redpandaconnectservice_stoppipeline[`PUT /v1/redpanda-connect/pipelines/\{id}/stop`] request.
...
To start a previously stopped pipeline, make a link:/api/doc/cloud-dataplane/operation/operation-redpandaconnectservice_startpipeline[`PUT /v1/redpanda-connect/pipelines/\{id}/start`] request.
...
To update a pipeline, make a link:/api/doc/cloud-dataplane/operation/operation-redpandaconnectservice_updatepipeline[`PUT /v1/redpanda-connect/pipelines/\{id}`] request. You update a pipeline configuration to scale resources, for example the number of CPU cores and amount of memory allocated.
...
. Use the link:/api/doc/cloud-dataplane/operation/operation-kafkaconnectservice_createsecret[Secrets API] to create a secret that stores the Base64-encoded secret data:
+
[,bash]
----
...
To create a connector, make a POST request to link:/api/doc/cloud-dataplane/operation/operation-kafkaconnectservice_createconnector[`/v1/kafka-connect/clusters/\{cluster_name}/connectors`]. 
...
To restart a connector, make a POST request to the link:/api/doc/cloud-dataplane/operation/operation-kafkaconnectservice_restartconnector[`/v1/kafka-connect/clusters/\{cluster_name}/connectors/\{name}/restart`] endpoint:
