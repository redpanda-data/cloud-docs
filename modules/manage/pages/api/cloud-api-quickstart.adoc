...
The following steps describe how to authenticate with the Cloud API and create a new Redpanda cluster. For more information on the Cloud API, see the link:/api/doc/cloud-controlplane/topic/topic-cloud-api-overview[Redpanda Cloud API Overview].
...
+
--
. In the Redpanda Cloud UI, create a https://cloud.redpanda.com/organization-iam?tab=service-accounts[service account (client)] and use the client ID and secret to request an API token. Use the token to link:/api/doc/cloud-controlplane/authentication[authenticate to the API].
. Create a resource group by making a link:/api/doc/cloud-controlplane/operation/operation-resourcegroupservice_createresourcegroup[`POST /v1/resource-groups`] request.
. Create a network by making a link:/api/doc/cloud-controlplane/operation/operation-networkservice_createnetwork[`POST /v1/networks`] request. Note that this operation may be long-running.
. Create a cluster by making a link:/api/doc/cloud-controlplane/operation/operation-clusterservice_createcluster[`POST /v1/clusters`] request.
