...
Providers tell Terraform how to communicate with the services you want to manage. For example, the Redpanda provider connects to the xref:api:ROOT:cloud-api.adoc[Redpanda Cloud API] using client credentials.
...
TIP: See the full list of zones and tiers available with each cloud provider in the xref:api:ROOT:cloud-api.adoc#api-description[API reference].
