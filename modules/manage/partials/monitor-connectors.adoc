...
A connector generally performs lower than expected when it is underprovisioned. In the connector configuration for a given number of instances and instance types, increase the number of `Max Tasks` (`tasks.max`).
