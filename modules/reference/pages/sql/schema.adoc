...
By default, Redpanda SQL uses the `public` schema. When you reference an unqualified `table_name`, that `table_name` is equivalent to `public.table_name`. The same rule applies to `CREATE`, `DROP`, and `SELECT TABLE` statements.
