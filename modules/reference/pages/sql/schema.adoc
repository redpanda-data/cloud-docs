...
By default, the `public` schema is used in Redpanda SQL. When unqualified `table++_++name` is used, that `table++_++name` is equivalent to `public.table++_++name`. It also applies to `CREATE`, `DROP`, and `SELECT TABLE` statements.
...
* `schema++_++name` is the schema name you are going to create.
...
* `schema++_++name` is the schema that you have created.
* `table++_++name` is the table name you are going to create.
...
* `schema++_++name` is the name of the schema.
* `table++_++name` is the name of the table you want to display.
