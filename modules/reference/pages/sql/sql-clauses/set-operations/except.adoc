...
* `value1, value2, ... value++_++n`: The columns you want to retrieve.
...
This result will show a `customer++_++id` who has only transacted in the first marketplace and has not engaged in any corresponding transactions in the second marketplace.
...
The diagram below shows a list of customer-product pairs found in the first marketplace (`marketplace1++_++transactions`) but missing in the second marketplace (`marketplace2++_++transactions`).

=== Example ++#++2

Let’s create two tables, `left++_++array++_++values` and `right++_++array++_++values`, to hold sets of values.
