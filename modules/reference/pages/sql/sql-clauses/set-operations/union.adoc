= UNION
:description: The UNION combines the result sets of 2 or more select statements, removing duplicate rows between the tables.
...
Consider an example of the `UNION`. Assume there is a table called `employees` and another table called `contractors`. The goal is to retrieve a combined list of names from both tables, excluding duplicates:
...
The result includes the values of both tables without any duplicate values.
...
The following diagram shows that the duplicate name "`Alice`" is represented only once in the output, fulfilling the requirement to avoid duplicate entries.
...
The syntax of the `UNION ALL` is as follows:
...
Combine all values from the tables by using `UNION ALL`:
...
In this case, it displays all the values of the first table followed by all the contents of the second table.
...
This diagram illustrates that with the `UNION ALL`, all values are displayed, including the duplicate ones.
