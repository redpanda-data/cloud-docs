...
* `value1, value2, ... value++_++n`: The columns you want to retrieve. You can also use `SELECT ++*++ FROM` to retrieve all columns.
...
Suppose you have two tables: `customers++_++old` and `customers++_++new`, containing customer data for different periods. You want to find the customers who are present in both tables:
