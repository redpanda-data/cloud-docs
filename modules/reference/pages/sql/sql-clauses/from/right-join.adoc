...
[TIP]
====
Table aliasing is supported in the `RIGHT JOIN` clause.
====
...
. Based on these tables, a `RIGHT JOIN` query can be written as follows:
+
[source,sql]
----
...
* If the records are equal, a new row is created with `customer_name` and `order_amount` columns as defined in the `SELECT` clause.
* `ELSE` still creates a new row with a `NULL` value from the left table (`customer`).
