...
. `FROM table++_++1`, defines the *left table* with table++_++1 in the FORM clause.
. `RIGHT JOIN table++_++2` defines the *right table* with table++_++2 in the RIGHT JOIN condition.
...
* Then it combines the values from the *orders* table using the *customer++_++id* and matches the records using the *id* column from the *customer* table.
* If the records are equal, a new row will be created with `customer++_++name` and `order++_++amount` columns as defined in the `SELECT` clause.
...
* The order id: `181897` doesn’t match with any customer. Thus the customer++_++name column is filled with: `null`.
