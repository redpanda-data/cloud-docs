...
[arabic]
. `SELECT column_1, column_2...` defines the *columns* from both tables where we want the data to be selected.
. `FROM table_1` defines the *left table* as the main table in the FORM clause.
. `RIGHT JOIN table_2` defines the *right table* as the table the main table joins.
