= LEFT JOIN
:description: The LEFT JOINreturns all matching records from the left table combined with the right table.
...
[TIP]
====
Table aliasing is supported in the `LEFT JOIN` clause.
====
...
{empty}1) Based on these tables, a `LEFT JOIN` query can be written as follows:
...
* If the records are equal, a new row is created with `item_no`, `item_name`, and `sold_qty`, `sold_price` columns as defined in the `SELECT` clause.
* `ELSE` it creates a new row with a `NULL` value from the right table (`invoice`).

{empty}2) This query returns the following result:
...
* The result displays all the item’s data from the left table (`item` table), even if there is 1 item that hasn’t been sold.
...
* The item id: `116` matches the item `Bread` and has been sold three times, for 6pcs/30000, 2pcs/10000, and 1pc/5000.
