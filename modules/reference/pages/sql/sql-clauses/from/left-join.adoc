...
[arabic]
. `SELECT column++_++1, column++_++2...` defines the *columns* from both tables where we want the data to be selected.
. `FROM table++_++1` defines the *left table* as the main table in the FORM clause.
. `RIGHT JOIN table++_++2` defines the *right table* as the table the main table joins.
...
{empty}1) Based on the above tables, we can write a `LEFT JOIN` query as follows:
...
* Then it combines the values from the *item* table using the *item++_++no* and matches the records using the *item* column of each row from the *invoice* table.
* If the records are equal, a new row will be created with `item++_++no`, *`item++_++name`*, and `sold++_++qty`, `sold++_++price` columns as defined in the `SELECT` clause.
* *ELSE* it will create a new row with a `NULL` value from the right table *(invoice)*.

{empty}2) The above query will give the following result:
...
* The item id: `113` matches the item `tea` but has never been sold. Thus the sold++_++qty & sold++_++price columns are filled with: null.
