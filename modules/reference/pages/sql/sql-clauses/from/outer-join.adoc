...
[arabic]
. `SELECT column++_++1, column++_++2...` defines the *columns* from both tables where we want to display data.
. `FROM table++_++1` represents the *left table* with table++_++1 in the FROM clause.
. `FULL OUTER JOIN table++_++2` represents the *right table* with table++_++2 in the FULL OUTER JOIN condition.
. `ON table++_++1.matching++_++field = table2.matching++_++field` sets the join condition after the *ON* keyword with the matching field between the two tables.
...
{empty}1) Based on the above tables, we can write an `OUTER JOIN` query as follows:
...
{empty}1) Let’s find out the employee who doesn’t belong to any department by adding a WHERE clause and NULL as the following query:
