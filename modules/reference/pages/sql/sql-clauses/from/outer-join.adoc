...
{empty}1) Based on the above tables, an `OUTER JOIN` query can be written as follows:
...
{empty}2) The result will show every department with an employee and the employee who works under a specific department.
...
----
+
{empty}2) The result will show that Gilbert Tom doesn’t belong to any department 👨🏻‍💼
+
[source,sql]
----
+------------------+--------------------+
