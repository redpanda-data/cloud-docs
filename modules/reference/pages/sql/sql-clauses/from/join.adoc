...
. Based on these tables, a `JOIN` query can be written as follows:
+
[source,sql]
----
...
+
If the values are equal, it creates a new row that contains columns from both tables (`category_name`) and adds the new row (`movie_name`) to the result set.
+
