...
* *`primary++_++statement`*: has to be one of the following: `SELECT`, `INSERT`, `UPDATE`, `DELETE`
* *`with++_++statement++_++body`*: has to be a `SELECT` statement (it can refer to aliases defined earlier in the query)
