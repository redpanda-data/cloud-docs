= WITH
:description: The WITH clause provides a way to define auxiliary statements (referred by their alias names), that can be used within a more complex query sets.
...
Currently, Redpanda SQL only supports not materialised CTEs (e.g. each auxiliary query alias is replaced with its corresponding body at the early stages of the query processing). The following query:
...
Both subqueries use the same auxiliary `math_grades` query, but each of them filters it using different keys. This way, both scans will only read a part of the table. If materialized CTE was used (which is not yet supported), the query engine would need to scan the whole table first and then filter the result twice, for each subquery.
...
Executing this query returns `2, 1` as output.

When referencing an alias, the context defined at the nested query level is used. If it does not define the referenced alias, the search moves up one level and repeats until an alias definition is found.
