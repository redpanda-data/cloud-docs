...
* `column++_++list`: The columns or calculations that you wish to retrieve.
* `table++_++name`: The tables that you want to retrieve records from.
...
* `LIMIT row++_++count`: It specifies a limited number of rows to be returned based on *row++_++count*.

=== Special Case

[arabic]
. If the `row++_++count` value is *NULL,* the query will produce a similar outcome because it does not contain the `LIMIT` clause.
. If `row++_++count` is *zero*, the statement will return an empty set.
...
This example uses the `LIMIT` clause to get the first four orders sorted by `order++_++id`:
...
* The orders with `order++_++id`= *1002 & 1003* aren’t displayed because we put the `OFFSET` value with 2. So the first 2 lines are ignored.
* The orders with `order++_++id`= *1009 & 1010* aren’t displayed because the `LIMIT` value is 5, which will display only 5 rows.
