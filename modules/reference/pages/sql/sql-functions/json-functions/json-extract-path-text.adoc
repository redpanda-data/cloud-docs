...
Besides the syntax above, Redpanda SQL provides and supports the use of operators in queries. See the syntax below:
...
. The `JSON_EXTRACT_PATH_TEXT()` function extracts the values and returns the output below:
+
[source,sql]
----
+------------+
