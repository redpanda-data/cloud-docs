...
The `GREATEST()` function extracts the greatest or largest value from a set of values. It needs at least one argument to work with, and if there are different types, like a text and a number, it returns an error.
...
The query will return `10`, the greatest value among the provided values.
...
The `GREATEST` function can also be used to find the Greatest value between column data. For example, create a table named *Student* that stores students’ names and scores.
...
Now find the greatest marks for every student in all subjects.
