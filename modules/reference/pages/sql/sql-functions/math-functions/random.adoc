...
The RANDOM() function generates a random number greater than or equal to zero but less than one by default. Use the following statement to retrieve a random number:
...
To generate a random decimal number between two values, use the following statement:
...
The following example shows how to retrieve a random number:
