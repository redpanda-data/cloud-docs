...
* `value++_++1`: Represents the first value.
* `value++_++n`: Represents one or more additional values, separated by commas.
...
This query will add a new column named `least++_++grade` to the result, displaying the smallest value among columns `x`, `y`, and `z`.
