...
The `LEAST()` function returns the least or smallest value in a list of values. It needs at least one argument to work with, and if you mix different types, like a text and a number, it returns an error.
...
The following are several examples of the `LEAST()` function:
