= DATE_TRUNC
:description: The DATE_TRUNC() function truncates a timestamp, timestamp with time zone or interval value to the specified precision, effectively rounding down the 

The `DATE++_++TRUNC()` function truncates a timestamp, timestamp with time zone or interval value to the specified precision, effectively rounding down the value to the start of the given time unit. The return type matches the input type.

== Syntax

The syntax for using the `DATE++_++TRUNC()` function is as follows:
...
* `time++_++zone` _(applicable for the second syntax option)_: The time zone for the operation. It accepts `text` input

== Fields

Below is a list of supported values to specify the fields param in `DATE++_++TRUNC()` syntax.
