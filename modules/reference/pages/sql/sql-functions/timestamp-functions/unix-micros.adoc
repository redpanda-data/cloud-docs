...
The `UNIX_MICROS()` function returns a given timestamp into a UNIX timestamp in microseconds, from 1970-01-01 00:00:00-00 (can be negative). Its syntax is as follows:
...
Suppose you have a table named *time_example* with the following timestamp values:
...
To convert all timestamp values into UNIX timestamp values in microseconds, run the following query:
