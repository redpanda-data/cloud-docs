...
The `UNIX++_++MICROS()` function returns a given timestamp into a UNIX timestamp in microseconds, from 1970-01-01 00:00:00-00 (can be negative). Its syntax is illustrated below:
...
=== Case ++#++1: Basic `UNIX++_++MICROS()` function

The below example uses the `UNIX++_++MICROS()` function to convert a given timestamp into a UNIX timestamp in microseconds:
...
=== Case ++#++2: `UNIX++_++MICROS()` function using columns

Let’s suppose we have a table named *time++_++example* with the following timestamp values:
...
The output displays all the timestamp entries in the *time++_++stamp* column and the converted UNIX timestamps in microseconds in the column *time++_++micros*.
