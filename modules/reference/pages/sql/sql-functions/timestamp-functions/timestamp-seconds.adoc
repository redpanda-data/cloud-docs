...
The `TIMESTAMP_SECONDS()` function converts a given UNIX timestamp value in seconds from 1970-01-01 00:00:00 UTC into a timestamp. Its syntax is as follows:
...
Suppose you have a table named **unix_time** with the following UNIX time values in seconds:
...
To convert all UNIX timestamp values in seconds to timestamp values, run the following query:
