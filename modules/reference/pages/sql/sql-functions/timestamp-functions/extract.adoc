...
The following table shows the supported input and corresponding return types for the `EXTRACT()` function:
...
The output is the month’s part of the given timestamp:
...
This example uses the `EXTRACT()` function to extract a given timestamp’s *SECONDS*:
...
The output is the seconds’ part of the given timestamp:
