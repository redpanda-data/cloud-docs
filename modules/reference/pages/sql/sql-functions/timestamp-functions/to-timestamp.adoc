...
* `source`: The date/time value to convert. Type is `TIMESTAMP` (`YYYY-MM-DD HH:MM:SS`).
* `format`: The format of the input string.

NOTE: If the source string is `NULL`, `TO_TIMESTAMP` returns `NULL` instead of raising an error. This behavior is compatible with PostgreSQL.
...
|`YYYY`
|Year (1–9999)
a|- The lowest possible value is 1 AD. +
  0001 is 1. +
  1 is 1.
