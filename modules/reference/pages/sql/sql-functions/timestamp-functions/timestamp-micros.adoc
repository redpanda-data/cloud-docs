...
The `TIMESTAMP_MICROS()` function converts a given UNIX timestamp value in microseconds since 1970-01-01 00:00:00 UTC into a timestamp. Its syntax is as follows:
...
Suppose you have a table named **timemicro_example** with the following UNIX time values in microseconds in the *unix_timestamp* column:
...
To convert all UNIX timestamp values in microseconds to timestamp values, run the following query:
