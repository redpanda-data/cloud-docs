...
The `COUNT()` function retrieves the number of records that match a specific condition. It can be used with any data type supported by Redpanda SQL, and returns a `BIGINT`.
...
The following example returns the number of all orders in the orders table:
