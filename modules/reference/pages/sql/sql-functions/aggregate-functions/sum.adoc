...
The following example uses the `SUM()` function to calculate the total rent price of all `rental++_++id`:
...
It will return a sum value of the `total++_++price`:
...
The following example uses the `SUM()` function to calculate the total rent price of the `customer++_++id = 11118.`
...
Since no records in the *rental* table have the `customer++_++id = 11118`, the `SUM()` function returns a `NULL`.
...
It will calculate the `total++_++price` from a group of `customer++_++id` as shown below:
