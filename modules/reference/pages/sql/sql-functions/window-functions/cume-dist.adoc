= CUME_DIST()
:description: The CUME_DIST() function is a window function used to calculate the cumulative distribution of a value within a set of values.

The `CUME++_++DIST()` function is a window function used to calculate the cumulative distribution of a value within a set of values. This function returns a value between 0 and 1, representing a relative position of a row within a partition or result set.
...
For the needs of this section we will use a simplified version of the `film` table from the link:https://www.postgresql.org/ftp/projects/pgFoundry/dbsamples/pagila/pagila/[Pagila database], containing only the `title`, `length` and `rating` columns.
...
The query below uses the `CUME++_++DIST()` function to calculate the cumulative distribution of film lengths:
