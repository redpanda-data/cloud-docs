...
The `FIRST++_++VALUE()` is a window function that retrieves the first value in an ordered set of values within a specified partition.
...
* `PARTITION BY`: optional clause, which divides the result set into partitions to which the `FIRST++_++VALUE()` function is applied (if omitted, the entire result set is treated as a single partition)
...
The query below uses the `FIRST++_++VALUE()` function to retrieve the title of the film with the shortest duration, partitioning results by rating and ordering by length.
