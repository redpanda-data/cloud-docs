...
The `BOOL++_++OR()` window function evaluates whether at least one value within a specified window of rows is `TRUE`.
...
The query below uses the `BOOL++_++OR()` function to evaluate whether at least one film in each rating category have a length greater than 150:
