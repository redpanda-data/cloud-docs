...
The `REGEXP_MATCH()` function can capture multiple groups within a match, which extracts key parts from a string in a structured way. The following example extracts the protocol, domain and path from a given URL:
...
Now, validate whether user emails in the `users` table are valid. If the regex doesn’t match, a `NULL` value is returned.
