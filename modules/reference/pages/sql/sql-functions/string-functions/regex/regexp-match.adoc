...
The `REGEXP++_++MATCH()` function matches a POSIX regular expression pattern to a string. It returns an array of `TEXT++[]++` type with substring(s) of matched groups within the first match.

== Syntax

The syntax for `REGEXP++_++MATCH()` function is as follows:
...
* `source++_++string`: string on which you want to perform the matching
* `pattern`: POSIX regular expression pattern to match
* `flags`: (optional) string with flags that change the matching behavior of `REGEXP++_++MATCH()` function
...
* `c`: `REGEXP++_++MATCH()` function is case sensitive by default, using the `c` flag has the same effect as having no flags at all
...
The `REGEXP++_++MATCH()` function can capture multiple groups within a match, which allow you to extract key parts from a string in a structured way. The example below extracts the protocol, domain and path from a given URL:
