...
Consider a table with the title *petsData*, as shown below.
...
From the table above, the following query retrieves the values of the *petname* column that start with “J”:
...
The following *petsData* table has a `NULL` value in the breed column.
...
The following result shows that `STARTS_WITH` returns true for all records (even the `null` one):
