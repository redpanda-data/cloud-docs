= STRPOS
:description: The STRPOS() is used to return the position from where the substring (the second argument) is matched with the string (the first argument).
...
The example below finds the *ut* (substring) position in the *computer* (string):
...
The following *listofwords* table stores word data.
