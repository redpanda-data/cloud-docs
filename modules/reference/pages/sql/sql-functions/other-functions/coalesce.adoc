= coalesce()
:description: Tables can hold null and non-null values.
:page-topic-type: reference

Tables can hold null and non-null values. The `COALESCE()` function is useful for ignoring null values while processing data, by returning the first argument that is not null, while the remaining arguments from the first non-null argument are not evaluated.
