...
:description: These trigonometric functions in Redpanda SQL take arguments and return values of type double precision and real.

These trigonometric functions in Redpanda SQL take arguments and return values of type `double precision` and `real`.
...
|`cosh` |It calculates the hyperbolic cosine of a given argument. |`cosh(argument)` |`select cosh(1);` It will return: `1.5430806348152437`
...
|`sinh` |It calculates the hyperbolic sine of a given argument. |`sinh(argument)` |`select sinh(1);` It will return: `1.1752011936438014`
