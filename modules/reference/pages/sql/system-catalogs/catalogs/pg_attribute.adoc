...
The `pg_attribute` stores information about table columns. It mimics the https://www.postgresql.org/docs/current/catalog-pg-attribute.html[pg_attribute^] PostgreSQL system catalog.

== Columns

[NOTE]
====
This table is designed for compatibility with tools that require PostgreSQL system tables, so it mostly has dummy data. Not all columns in `pg_attribute` are applicable to every type of relation
====
...
. To filter columns, use `pg_attribute` with a `WHERE` clause:
+
[source,sql]
----
SELECT attrelid, attname, atttypid, attnum
