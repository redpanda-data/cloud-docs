...
The `pg++_++description` stores descriptions (comments) for each database object. It mimics the https://www.postgresql.org/docs/current/catalog-pg-description.html[pg++_++description] PostgreSQL system catalog.
...
====
The following columns are available for querying in `pg++_++description`:
