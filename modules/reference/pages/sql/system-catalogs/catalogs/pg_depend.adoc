...
The `pg++_++depend` tracks relationships between database objects, such as tables, columns, constraints, and indexes. It mimics the https://www.postgresql.org/docs/current/catalog-pg-depend.html[pg++_++depend] PostgreSQL system catalog.
...
====
The following columns are available for querying in `pg++_++depend`:
