...
The `pg++_++constraint` stores information about table constraints. It mimics the https://www.postgresql.org/docs/current/catalog-pg-constraint.html[pg++_++constraint] PostgreSQL system catalog.
...
====
The following columns are available for querying in `pg++_++constraint`:
