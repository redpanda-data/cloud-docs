...
A user cannot be dropped while they own objects or hold privilege grants. Reassign or remove those objects and revoke the grants first, then drop the user. If the user also owns objects (for example, a catalog or table created by that user), reassign or drop those before running `DROP USER`.
