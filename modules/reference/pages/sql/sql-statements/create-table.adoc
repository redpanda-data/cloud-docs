...
Map the `transactions` topic to a table through `production_redpanda`:

[source,sql]
----
CREATE TABLE production_redpanda=>transactions
...
[source,sql]
----
CREATE TABLE IF NOT EXISTS production_redpanda=>sensor_readings
WITH (
  topic = 'sensor-data',
