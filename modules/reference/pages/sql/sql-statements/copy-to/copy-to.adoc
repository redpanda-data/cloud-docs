= COPY TO
:description: The COPY TO statement is used to export tables, specific columns, or results of select queries into .csv files.
...
* `table++_++name`: Table with the data to export.
* `column++_++name`: Optional. Specify columns for export.
...
[arabic, start=2]
. You will receive a list of existing tables in Redpanda SQL:
