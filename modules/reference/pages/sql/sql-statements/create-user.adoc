...
[NOTE]
====
Error messages from the SQL engine refer to users as "roles" (for example, `role "alice" does not exist`). This is consistent with PostgreSQL terminology and refers to the same object that `CREATE USER` produces.
====
