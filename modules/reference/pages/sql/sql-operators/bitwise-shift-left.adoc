...
When performing bitwise left shift operations (<<) on 32-bit integer values in Redpanda SQL, the shift count is taken *modulo* 32. This means: * Shifting by a number of bits greater than or equal to 32 will wrap around * For example, `1 << 35` is equivalent to `1 << 3` because `35`latexmath:[modulo]`32 = 3`
