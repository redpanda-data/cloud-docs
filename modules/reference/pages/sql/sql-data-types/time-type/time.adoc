= Time
:description: The TIME data type in Oxla stores time values without any date information.
...
Let’s create a table to manage employee schedules, containing their names and the time they are scheduled to start work. The TIME data type will be used for the `start++_++time` column.
...
To view all employee schedules in the `employee++_++schedule` table, we can use the `SELECT` statement.
