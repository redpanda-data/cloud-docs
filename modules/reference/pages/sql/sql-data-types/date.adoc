= Date
:description: The DATE data type is used to store and insert date values.
...
In this example, we will create an *emp++_++submission* table that consists of the candidate ID, candidate name, the submitted department, and a submission date with a `DATE` data type.
...
The following is the result of the `SELECT` statement where the values in the `sub++_++date` column have `DATE` data type:
