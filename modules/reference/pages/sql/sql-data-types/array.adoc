= Array
:description: In Oxla, an array allows you to represent a collection of elements that have the same data type (any built-in data type can be used).
...
====
Arrays can also be accessed by using array slices. An array slice is denoted by writing `lower++_++bound:upper++_++bound`. The bounds can be omitted, in which case the slice is unbounded from a given side:
