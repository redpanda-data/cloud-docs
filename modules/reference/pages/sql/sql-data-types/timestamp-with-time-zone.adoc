...
Redpanda SQL provides you with two data types for handling timestamps:
...
*Important Note:*
 ...
 The timezone information is not stored in the database. Consequently, every time a user requests a value of this type, Redpanda SQL converts from UTC to the user's local timezone before displaying it.
====
[WARNING]
====
*Important Note:*
 Redpanda SQL relies on timezone information served by host machine operating system. It must be up-to-date in order to ensure correct timestamp conversions, date calculations, and compliance with regional time changes such as daylight saving adjustments.
====
...
It will return the result as displayed below. Notice that the `event_timestamp_tz` is converted to UTC timezone.
...
To sort the events based on the `event_timestamp` column and display the corresponding UTC in the `event_timestamp_tz` column, run the following query:
