...
// TODO: At Cloud GA the IdP is Auth0 (single-IdP architecture per RFC + SME
// comments). External IdP federation and customer-owned IdPs are not supported.
// Once federation lands, document the validated external IdPs here.
...
Two distinct cluster settings affect TLS:
...
+
// TODO: Confirm the correct cross-link for the cluster SSL mode setting in Redpanda Cloud. The previous reference to `security:authorization/gbac/gbac_dp.adoc` was misdirected (GBAC covers group-claim mapping, not SSL mode).
* `oidc.require_tls` (default `true`) controls whether Redpanda SQL uses HTTPS when contacting Redpanda Cloud's identity provider: the OIDC discovery document URL, the JWKS endpoint, and the token endpoint for the client-credentials flow. This setting does not gate TLS on the client's `psql` connection.
