...
In Redpanda SQL, most errors that would crash a server should instead put it into a degraded state. The following are key terms related to the node or cluster state:
...
Temporary degradation occurs when a node cannot perform queries because it waits for specific conditions. The following errors are related to a temporary degraded state:
