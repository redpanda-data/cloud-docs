...
Access to Redpanda SQL is controlled through authentication (Redpanda Cloud glossterm:OpenID Connect (OIDC)[,OIDC], or SCRAM for legacy clients) and authorization (xref:security:authorization/rbac/rbac_dp.adoc[data-plane RBAC roles] plus per-resource `GRANT` statements).
