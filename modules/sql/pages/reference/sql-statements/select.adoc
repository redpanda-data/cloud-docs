...
* `table++_++name` represents the table(s) name.
...
We have a table named *student++_++data* that stores the id, name, and where the student lives.
...
[arabic]
. In the first case, we want to display all the data from the *student++_++data* table. Please refer to the syntax below:
...
[arabic, start=3]
. If you have successfully run the query, you will get all the data from the *student++_++data* table.
...
[arabic, start=3]
. If you have successfully run the query, you will get a list of students’ IDs & names from the *student++_++data* table.
