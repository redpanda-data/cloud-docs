...
The `REGEXP++_++REPLACE()` function replaces all occurrences of a regular expression pattern in a string with a specified replacement string.

=== Syntax

The syntax for `REGEXP++_++REPLACE()` function is as follows:
...
* `source++_++string`: string that we want to perform the replacement on
* `pattern`: POSIX regular expression pattern to match
* `replacement`: replacement string
* `flags`: (optional) string that changes the matching behavior of `REGEXP++_++REPLACE()` function
...
* `c`: `REGEXP++_++REPLACE()` function is case sensitive by default, using the `c` flag has the same effect as using no flags

=== Examples

==== Basic function usage

In this example, we will focus on using `REGEXP++_++REPLACE()` function with a basic POSIX regular expression pattern:
...
This example will focus on using the `REGEXP++_++REPLACE()` function with a defined flag and replacing certain substrings in a string. For the needs of this section, we will create a sample `quotes` table:
...
Now, we will use the `REGEXP++_++REPLACE()` function with the `i` flag specifiec to replace all occurrences of the word `lazy` with `active` regardless of the case sensitivity:
...
Without specifying the `g` flag, `REGEXP++_++REPLACE()` function replaces only the first occurrence of a substring:
...
* If the regular expression pattern isn’t found in the string, the `REGEXP++_++REPLACE()` function returns the original string
