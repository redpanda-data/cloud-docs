...
Let’s see how the `LENGTH()` function works on the *personal++_++details* table containing the employee’s *id*, *first++_++name*, *last++_++name*, and *gender* of a retail store as columns.
...
The following query returns the last name and the length of the last name from the personal++_++details table, where the length of the last++_++name is greater than 5.
...
The output displays all those items in the last++_++name column with a length of more than 5 characters.
