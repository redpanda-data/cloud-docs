...
Let’s see how the `UPPER()` function works using an example with columns. We have a table named *personal++_++details* containing employee’s *id*, *first++_++name*, *last++_++name*, and *gender* of a retail store:
...
. Then, combine them using the `CONCAT()` function into one *full++_++name* column in uppercase.
