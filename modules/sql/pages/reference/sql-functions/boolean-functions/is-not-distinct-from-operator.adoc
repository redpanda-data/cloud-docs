...
Suppose we have a table named customer++_++contacts that stores customer contact information.
...
In this query, we retrieve all rows from the `customer++_++contacts table` where the email and phone are NULL. We can conclude that the customer with `customer++_++id 104` has no phone number or email address.
