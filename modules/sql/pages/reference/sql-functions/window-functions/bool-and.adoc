= BOOL_AND()
:description: The BOOL_AND() window function evaluates whether all values within a specified window of rows are TRUE.

The `BOOL++_++AND()` window function evaluates whether all values within a specified window of rows are `TRUE`.
...
The query below uses the `BOOL++_++AND()` function to evaluate if all films in each rating category have a length greater than 100:
