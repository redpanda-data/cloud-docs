= NTH_VALUE()
:description: The NTH_VALUE() is a window function that allows you to access the value from the nth row within a specified window frame.

The `NTH++_++VALUE()` is a window function that allows you to access the value from the nth row within a specified window frame.
...
* `PARTITION BY`: optional clause, which divides the result set into partitions to which the `NTH++_++VALUE()` function is applied (if omitted, the entire result set is treated as a single partition)
...
The query below uses the `NTH++_++VALUE()` function to retrieve the title of the film with the second shortest duration, partitioning results by rating and ordering by length:
