= COUNT()
:description: The COUNT() window function allows you to retrieve the number of records that meet a specific criteria.

The `COUNT()` window function allows you to retrieve the number of records that meet a specific criteria. When using it with he `RANGE` clause, it allows you to perform counts within a defined range based on the values of the current row. This function can be used with all xref:sql:reference/sql-data-types/index.adoc[data types supported by Oxla].
...
`sql COUNT RANGE COUNT(expression) OVER (   ++[++PARTITION BY partition++_++expression++]++   ORDER BY sort++_++expression   ++[++ROWS ++|++ RANGE BETWEEN start++_++value AND end++_++value++]++ )`
...
This query above counts the number of sales transactions within a 90-day window before each `dateid`, including the current sale:
