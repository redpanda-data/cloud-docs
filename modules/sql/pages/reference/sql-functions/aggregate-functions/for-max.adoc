= FOR_MAX()
:description: FOR_MAX() function is used to search for a maximum in a specific column and return a value related to that maximum from another column.

`FOR++_++MAX()` function is used to search for a maximum in a specific column and return a value related to that maximum from another column.
...
The `FOR++_++MAX()` function returns `NULL` in the following situations:
...
==== `FOR++_++MAX()` with `GROUP BY` clause

In this example, we will use a `GROUP BY` clause to group customers and then utilise the `FOR++_++MAX()` function to get a discount for the highest price paid by each customer:
