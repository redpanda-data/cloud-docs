= FOR_MIN()
:description: The FOR_MIN() function is used to search for a minimum in a specific column and return a value related to that minimum from another column.

The `FOR++_++MIN()` function is used to search for a minimum in a specific column and return a value related to that minimum from another column.
...
The `FOR++_++MIN()` function returns `NULL` in the following situations:
...
==== `FOR++_++MIN()` with `GROUP BY` clause

To determine the discount associated with the lowest price paid by each customer, we will use the `GROUP BY` clause with `FOR++_++MIN()` function:
