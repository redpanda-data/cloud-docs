...
The `REGR++_++AVGY()` aggregate function calculates the mean of the dependent variable (y) for non-null pairs of dependent (y) and independent (x) variables. This function is used in linear regression analysis to compute the average value of the dependent variable where both variables are not null.
...
They query below uses the `REGR++_++AVGY()` function to calculate the mean of the dependent variable (`rating`) for rows where both `rating` and `length` are not null:
