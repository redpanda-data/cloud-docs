= VAR_SAMP()
:description: The VAR_SAMP() aggregate function calculates the sample variance of a set of numeric values.

The `VAR++_++SAMP()` aggregate function calculates the sample variance of a set of numeric values. This function measures the spread of data points around the mean, assuming the data is a sample of a larger population.
...
They query below uses the `VAR++_++SAMP()` function to calculate the sample variance for the `length` column where `rating` is greater than or equal to 4:
