...
The `TIMESTAMP++_++MICROS()` function converts a given UNIX timestamp value in microseconds since 1970-01-01 00:00:00 UTC into a timestamp. Its syntax can be seen below:
...
==== Case ++#++1: Basic `TIMESTAMP++_++MICROS()` function

The below example uses the `TIMESTAMP++_++MICROS()` function to convert a given UNIX timestamp in microseconds into a timestamp without a timezone:
...
==== Case ++#++2: `TIMESTAMP++_++MICROS()` function using columns

Let’s suppose we have a table named **timemirco++_++example** with the following UNIX time values in microseconds in the *unix++_++timestamp* column:
...
The output displays all the entries in the table in UNIX timestamp format (in microseconds) in the *unix++_++timestamp* column and in the timestamp format in the column *timestamp++_++value* without timezone:
