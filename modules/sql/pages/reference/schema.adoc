...
By default, the `public` schema is used in Oxla. When unqualified `table++_++name` is used, that `table++_++name` is equivalent to `public.table++_++name`. It also applies to `CREATE`, `DROP`, and `SELECT TABLE` statements.
