...
* `input++_++timestamp`: This represents the date and time value you want to convert. The user’s time zone is fixed to UTC.
* `TIME++_++ZONE`: The target time zone to which the timestamp will be converted.
...
Let’s consider from the xref:sql:reference/sql-data-types/timestamp-without-time-zone.adoc[visitors] table, we wish to retrieve a list of visit dates in the MST time zone. We can achieve this using the following query:
