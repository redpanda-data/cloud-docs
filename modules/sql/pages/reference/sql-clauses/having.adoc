...
The `HAVING` clause specifies a search condition by using an xref:sql:reference/sql-functions/aggregate-functions/index.adoc[aggregate function]. It will filter out the records returned from a `GROUP BY` clause that do not fulfill a specified condition.
