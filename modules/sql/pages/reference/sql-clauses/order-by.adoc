...
* `table++_++name`: table that you want to retrieve records from.
...
The following statement selects the employee name and employee salary from the *salaryemp* table and sorts the records in the `emp++_++sal` column in descending order:
...
For this example, we’ll create a new table called `interval++_++data`:
