...
* Confirm that the Redpanda topic you want to query has a schema registered in Schema Registry.

// TODO: Confirm permissions/roles/ACLs required
// Is it possible to use a topic without a registered schema? 
// Any specific limitations on Protobuf vs JSON vs Avro formats?
...
// TODO: Nested fields?
The table inherits its column definitions from the registered schema. For Protobuf schemas, Redpanda SQL maps each top-level field to a SQL column.
