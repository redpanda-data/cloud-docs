...
When Redpanda SQL is enabled on your cluster, a default Redpanda catalog named `default_redpanda_connection` is created automatically. Use this catalog to map your Redpanda topics as SQL tables and query them.
...
Map a Redpanda topic as a SQL table through `default_redpanda_connection`:

[source,sql]
----
CREATE TABLE default_redpanda_connection=>user_events
...
[source,sql]
----
SELECT * FROM default_redpanda_connection=>user_events LIMIT 10;
----
