...
With glossterm:Iceberg topic[,Iceberg-enabled topics], you can retain streaming data in your lakehouse beyond your Redpanda topic retention window and continue to query it with SQL. A single query returns both the live records still in the topic and the Iceberg-committed history that has aged out under topic retention, in one result.
...
When Redpanda SQL maps an Iceberg-enabled topic as a SQL table, it reads the Iceberg schema and converts each column type to a SQL type. The following column types currently have no valid SQL type mapping and cause `CREATE TABLE` or `REFRESH` to fail with an error.
